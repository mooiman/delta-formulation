...
\newcommand{\Fabs}{\textsl{Fabs}\xspace}  % Continue function for the absolute value
...
\nomenclature{$c_\Psi$}{$1\ {(.)}^{-1}$}{Artificial smoothing variable}
\nomenclature{$\vec{E}$}{-}{Error vector function, defined in computational space}
\nomenclature{$\zeta$}{\si{\metre}}{Water level above reference plane, positive upward}
\nomenclature{$z_b$}{\si{\metre}}{Bed level above reference plane, positive upward}
\nomenclature{$g$}{\si{\metre\per\square\second}}{Gravitational constant}
\nomenclature{$\theta$}{-}{$\theta$-method. If $\theta=1$ then it is a fully implicit method and if $\theta=0$ then it
     is a fully explicit method.}
