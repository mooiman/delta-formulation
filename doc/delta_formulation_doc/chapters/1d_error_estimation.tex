...
So if the numerical error needs to be smaller then 1\ \%, the number of grid cells per wavelength ($\lambda = N\Dx$) is computed as
\begin{align}
...
Given the wavelength from the required numerical error (1\ \%, $N = 13$), the coefficient $c_{\Psi}$ reads:
\begin{align}
c_{\Psi} = \frac{1}{(k\Dx)^2} \quad \Rightarrow \quad c_{\Psi} = \frac{N^2 }{(2\pi)^2} \approx 4
\end{align}
%
%--- begin light blue table ---
\begin{longtable}{|>{\bfseries}p{6mm-12pt}|p{\textwidth/4-6mm-12pt}|p{\textwidth/4-6mm-12pt}|p{\textwidth/4+3mm-12pt}|p{\textwidth/4+3mm-12pt}|}
...
\label{tab:numerical_accuracy_N}}
% Python: accuracy_table.py
\\%
 \rowcolor{mgreen1}
& {\textcolor{white}{\textbf{Accuracy}}}
& {\textcolor{white}{$\mathbf{(k\Dx)^2}$}}
...
\bottomline
\endlastfoot
1 & 5\ \%   & 1.20 &  0.8333  ($\approx$ 1)& 5.7357  ($\approx$ 6)\\
\midline
2 & 2\ \%  & 0.48 & 2.0833 ($\approx$ 2)   & 9.0690  ($\approx$ 10)\\
\midline
\rowcolor{mgreen2}
3 & 1\ \%  & 0.24 &  4.1667 ($\approx$ 4)   & 12.8255  ($\approx$ 13)\\
\midline
4 & 0.5\ \% & 0.12 & 8.3333 ($\approx$ 8)  & 18.1380  ($\approx$ 20)\\
\end{longtable}
...
= E
\end{align}
for constant $\Dx$ and constant $c_\Psi$ (see \autoref{tab:numerical_accuracy_c_error})
\begin{align}
    E = D^{u,x}_{\xi} = \Dx^2 \pdiff[2]{u}{x},
\end{align}
thus $E$ and $u$ do have the same dimension.
...
\begin{align}
    c_\Psi \Dx^2  =  c_{E}  \Dx^2  (k\Dx)^2 \frac{2}{\pi}
    \quad \Rightarrow \quad
    c_{E} = c_\Psi \frac{1}{(k\Dx)^2}\frac{\pi}{2}
\end{align}
For a $1$~\% accuracy, $c_\psi = 4$ see \autoref{tab:numerical_accuracy_c_error}, the maximum allowed Fourier modes is $k\Dx < 0.4899$, substitution yields:
\begin{align}
    c_E = c_\Psi \frac{1}{(k\Dx)^2}\frac{\pi}{2} \approx 4.1667 \frac{1}{0.24} \frac{\pi}{2}  \quad \Rightarrow \quad c_{E} \approx 27.2708
\end{align}
%
%--- begin light blue table ---
\begin{longtable}{|>{\bfseries}p{6mm-12pt}|p{\textwidth/4-6mm-12pt}|p{\textwidth/4-6mm-12pt}|p{\textwidth/4+3mm-12pt}|p{\textwidth/4+3mm-12pt}|}
\caption{Several typical values for numerical accuracy, $c_{\psi}$ and number $c_E$, the highligthed line is set as default.
    \label{tab:numerical_accuracy_c_error}}
% Python: accuracy_table.py
\\%
\rowcolor{mgreen1}
& {\textcolor{white}{\textbf{Accuracy}}}
...
\bottomline
\endlastfoot
1 & 5\ \%   &  1.20 & 0.8333  ($\approx$ 1)&  1.0908 ($\lesssim$  2  $c_\Psi)$ \\
\midline
2 & 2\ \%  & 0.48 & 2.0833 ($\approx$ 2)   &  6.8177 ($\lesssim$  4 $c_\Psi)$ \\
\midline
\rowcolor{mgreen2}
3 & 1\ \%  & 0.24 &  4.1667 ($\approx 4$)   & 27.2708 ($\lesssim$  7  $c_\Psi)$ \\
\midline
4 & 0.5\ \% & 0.12 & 8.3333 ($\approx$ 8)  &  109.0831 ($\lesssim$  14 $c_\Psi)$ \\
\end{longtable}
%--- end light blue table ---
%
%
