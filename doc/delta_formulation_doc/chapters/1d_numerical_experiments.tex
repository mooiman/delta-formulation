...
\begin{align}
    c(0,t) = c_{\it given}(t)
    \begin{cases}
        \frac{\exp(-1/\tau) }{ \exp(-1/\tau) + \exp(-1 / (1 - \tau))},
        &\quad \text{if\ }0 \leq \tau \leq 1,
        \\
        1 &\quad \text{if\ }\tau > 1,
    \end{cases}
...
\begin{symbollist}
    ...
    \item[$\tau$] Relative time to prescribe the transition from initial condition to boundary condition, $\tau = t/t_{reg}$.
    \item[$c_{\it given}(t)$] Prescribed (given) time-series at the boundary.
\end{symbollist}
%--------------------------------------------------------------------------------
...
\begin{figure}[H]
    ...
    \caption{Constituent at $t=\bqty{3600}{\second}$. Range of $y$-axis is from $ 1 \pm \num{5e-08}$.}
\end{figure}
...
The initial condition are computed as described in \autoref{sec:compatible_initialization_1d}
...
\end{figure}
The graphs in \autoref{fig:1d_adv_initial_wave_package} do not coincide on the grid points because the profiles are computed according \autoref{sec:compatible_initialization_1d}.
...
In this section we will show the resuls of the advection-diffusion equation with  Dirichlet boundary conditions at both sides of the domain and an interface problem, i.e.\ a large jump of the diffusion coefficient in the middle of the domain.
...
To force an outflow boundary layer at both boundaries a Dirichlet boundary is prescribed.
...
\end{figure}
%--------------------------------------------------------------------------------
\section{Burgers equation}\label{sec:1d_numerical_burgers_experiment}
In this section we will show the resuls of the viscid Burgers equation with  a Dirichlet boundary conditions at the left/west side of the domain and an outflow boundary at the rigth/east side of the domain.
...
\begin{symbollist}
    ...
    \item[$\nu$] Constant viscosity coefficient, $\bunit{\square\metre\per\second}$.
    \item[$\Psi$] Artificial viscosity coefficient, $\bunit{\square\metre\per\second}$.
\end{symbollist}
...
%         0 & \quad \text{if } \tau  < 0,
%         \\
           \frac{\phi(\tau) }{ \phi(\tau) + \phi(1-\tau)},
&\quad \text{if\ }0 \leq \tau \leq 1,
        \\
        1 &\quad \text{if\ }\tau > 1,
    \end{cases}
\\
    \phi(\tau) &= \exp(-1 / \tau ) \quad \text{with} \quad \tau = \frac{t}{t_{reg}}
    \label{eq:1d_burgers_boundary_condition}
\end{align}
where
\begin{symbollist}
    \item[$t_{reg}$] The regularization time for the given time-series, [\si{\second}]
\nomenclature{$t_{reg}$}{\si{\second}}{The regularization time for the given time-series}
    \item[$\tau$] Normalized time to prescribe the transition from initial condition to boundary conditionn \bunit{-}.
    \nomenclature{$\tau$}{\si{-}}{Normalized time to prescribe the transition from initial condition to boundary condition, i.e.\ $t_{reg}$, $0 \leq \tau \leq 1$}
     \item[$u_{\it given}(t)$] Prescribed (given) time-series at the boundary, \bunit{\metre\per\second}.
\end{symbollist}


%--------------------------------------------------------------------------------
\subsection{Smooth step initial condition, standing shock wave}
The smooth step initial condition has less numerical error on the boudnaries because derivatives at the boundaries are zero and the boundary conditions are chosen to generate a standing shock wave at the middle of the domain.
The smooth step intial condition reads:
\begin{align}
    u(x^*,0) &= u_l +
    (u_r - u_l) \frac{\phi(x^*) }{ \phi(x^*) + \phi(1-x^*)}
\label{eq:smoothstep}
\\
\phi(x^*) &= \exp(-1 / x^* ) \quad \text{with} \quad x^* = \frac{x}{L_x}
\end{align}
with
\begin{symbollist}
    \item[$x^*$] Normalized length with respect to $L_x$, \bunit{-}.
        \nomenclature{$x^*$}{\si{-}}{Normalized coordinate, $0 \leq x^* \leq 1$}
    \item[$L_x$] Length of the domain, \bunit{\metre}.
    \item[$u_l$] Boundary condition at left/west boundary of the domain, \bunit{\metre\per\second}.
    \item[$u_r$] Boundary condition at right/east boundary of the domain, \bunit{\metre\per\second}.
\end{symbollist}

%--------------------------------------------------------------------------------
\paragraph*{Analytic solution of standing shock wave}
The analytic solution for a fully developed standing shock wave reads (\citet[eq.\ 4.23]{Whitham1999}:
\begin{align}
    u(x,t) = u_r + (u_l - u_r)/
    \left(1 + \exp\left(\frac{(u_l - u_r)}{2\nu}(x-U_{s}t)\right)\right)
\end{align}
with
\begin{symbollist}
    \item[$u_l$] Boundary condition at left/west boundary of the domain, \bunit{\metre\per\second}.
    \item[$u_r$] Boundary condition at right/east boundary of the domain, \bunit{\metre\per\second}.
    \item[$U_{s}$] Velocity of the shock wave, i.e.\ $U_{s} = (u_l + u_r)/2$, \bunit{\metre\per\second}
    \nomenclature{$U_{s}$}{\si{\metre\per\second}}{Velocity of the shock wave which appears in Burgers equation}
\end{symbollist}
A standing shock wave at $x=0$ will appear if $U_s = 0$, thus $u_l = -u_r$.
A transformation of the $x$-coordinate is applied to get the shock wave in the middle of the domain.
%--------------------------------------------------------------------------------
\paragraph*{Numerical experiment}
The numerical experiment is performed with the following parameters:
\begin{itemize}
    \item Length of the domain, $L_x = \bqty{20000}{\metre}$
    \item Grid size, $\Dx \in \{5, 10, 25, 50, 100, 250\}$\ $\bunit{\metre}$
    \item Start time, $t_{start} = \bqty{0}{\second}$
    \item End time, $t_{stop} = \bqty{6}{\hour}$
    \item Timestep, $\Dt \in \{1, 2, 5, 10, 20, 60\}$ $\bunit{\second}$
    \item Regularization time for time-series, $t_{\it reg} = \bqty{1800}{\second}$
    \item Prescribed initial velocity  (\autoref{eq:smoothstep}), smooth step velocity profile between $u(0,t) = \bqty{1}{\metre\per\second}$ and $x=L_x$ by $u(L_x,t) = \bqty{-1}{\metre\per\second}$.
    \item Prescribed velocity at boundary on $x=0$ is given by $u(0,t) = \bqty{1}{\metre\per\second}$ and $x=L_x$ by $u(L_x,t) = \bqty{-1}{\metre\per\second}$.
    \item Viscosity coefficient $\nu = \bqty{0.1}{\metre\square\per\second}$.
\end{itemize}
%--------------------------------------------------------------------------------
\subsubsection{Results of the numerical experiments}
Results are presented for different $\Dx$ and different $\Dt$.
The results are more the less stationary over the time interval $t\in[3, 6]$ hour.
\begin{figure}[H]
        \centering
        \includegraphics[width=0.95\textwidth]{figures/standing_shock_wave_time_series.pdf}
        \caption{Time history for a simulation with
        $\Dx =\bqty{5}{\metre}, \Dt=\bqty{1}{\second}$ and
        $\Dx = \bqty{250}{\metre}, \Dt=\bqty{60}{\second}$.
        }
\end{figure}
Influence of $\Dt$ on results
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/standing_shock_wave_time_series_dt60_dt30_dt15.pdf}
    \caption{Time histories for simulations with
        $\Dx =\bqty{250}{\metre}, \Dt=\bqty{15}{\second}$,
        $\Dx =\bqty{250}{\metre}, \Dt=\bqty{30}{\second}$ and
        $\Dx = \bqty{250}{\metre}, \Dt=\bqty{60}{\second}$ at location $\bqty{7500}{\metre}$ from the origin.
    }
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/standing_shock_wave_velocity.png}
    \caption{Shock wave around $x=\bqty{10000}{\metre}$ with  analytic solution (blackline) and for simulations with:
        gray line $\Dx =\bqty{5}{\metre}, \Dt=\bqty{1}{\second}$;
        blue line $\Dx =\bqty{10}{\metre}, \Dt=\bqty{2}{\second}$;
        cyan line $\Dx =\bqty{25}{\metre}, \Dt=\bqty{5}{\second}$;
        red line $\Dx =\bqty{50}{\metre}, \Dt=\bqty{10}{\second}$;
        green line $\Dx =\bqty{100}{\metre}, \Dt=\bqty{20}{\second}$;
        blue line $\Dx =\bqty{250}{\metre}, \Dt=\bqty{60}{\second}$.}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/standing_shock_wave_psi.png}
    \caption{Used artificial viscosity $\Psi$  around $x=\bqty{10000}{\metre}$ for simulations with:
        gray line $\Dx =\bqty{5}{\metre}, \Dt=\bqty{1}{\second}$;
        blue line $\Dx =\bqty{10}{\metre}, \Dt=\bqty{2}{\second}$;
        cyan line $\Dx =\bqty{25}{\metre}, \Dt=\bqty{5}{\second}$;
        red line $\Dx =\bqty{50}{\metre}, \Dt=\bqty{10}{\second}$;
...
\begin{itemize}
    ...
    \item Timestep, $\Dt = \bqty{10}{\second}$
    \item Regularization time for time-series, $t_{\it reg} = \bqty{1800}{\second}$
    \item Prescribed initial velocity, $u_{\it initial}(x,0) = \bqty{1}{\metre\per\second}$
    \item Prescribed velocity on boundary at $x=0$ is given by \autoref{eq:1d_burgers_boundary_condition}, with $u(0,t) = \bqty{2}{\metre\per\second}$.
    \item No boundary is prescribed at $x=\bqty{20000}{\metre}$, free outflow.
\end{itemize}
The speed of the fully developed shock wave for the Burgers equation is $u_{\it shock} =\bqty{1.5}{\metre\per\second}$, see \citet[eq.\ 8.1.67]{Debnath2008}, \citet[eq.\ 11.23]{LeVeque2002}.
%--------------------------------------------------------------------------------
\subsubsection{Results of the numerical experiments}
%----------------------------------------------------------
\subparagraph*[Experiment with viscosity of 10 m2/s]{Experiment with viscosity of \bqty{10}{\metre\square\per\second}}
\vspace{1.0\baselineskip}
\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/nu10d0_velocity.png}
    ...
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/nu10d0_velocity_psi.png}
        \caption{Used artificial viscosity $\Psi$ along channel at $\bqty{2}{\hour}$. The blue line, no artificial viscosity ($\Psi = 0$) and the red line the used artificial viscosity.}
    \end{subfigure}
    \caption{Transect for velocity $\nu = \bqty{10}{\square\meter\per\second}$}
\end{figure}

\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/20260709_213744.pdf}
        \caption{Time histories of the velocity for the observation point  at the centre of the domain, $x=\bqty{10000}{\metre}$.
        The blue line, without artificial viscosity and the red line with artificial velocity.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/20260709_214125.pdf}
        \caption{Time histories of the artificial viscosity $\Psi$ for the observation point at the centre of the domain, $x=\bqty{10000}{\metre}$.
        The blue line, without artificial viscosity and the red line with artificial velocity.}
    \end{subfigure}
    \caption{Time histories for $\nu = \bqty{10}{\square\meter\per\second}$}
\end{figure}
The shock wave has a speed of $u_{\it shock} = \bqty{1.5152}{\metre\per\second}$ for both cases, without and with artificial viscosity.
%--------------------------------------------------------------------------------
\subparagraph*[Experiment with viscosity of 0.1 m2/s]{Experiment with viscosity of \bqty{0.1}{\metre\square\per\second}}
\vspace{1.0\baselineskip}
\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/nu00d1_velocity.png}
        \caption{Velocity along channel at $\bqty{2}{\hour}$.
            The blue line, without artificial viscosity and the red line with artificial velocity.\newline \phantom{text} }
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/nu00d1_velocity_psi.png}
        \caption{Used artificial viscosity $\Psi$ along channel at $\bqty{2}{\hour}$. The blue line, no artificial viscosity ($\Psi = 0$) and the red line the used artificial viscosity.}
    \end{subfigure}
...
\begin{figure}[H]
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/20260709_223603_no_psi.pdf}
        \caption{Time histories of the velocity for the observation point  at the centre of the domain, $x=\bqty{10000}{\metre}$.
        The blue line, without artificial viscosity and the red line with artificial velocity.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=\textwidth]{figures/20260709_223946_psi.pdf}
        \caption{Time histories of the artificial viscosity $\Psi$ for the observation point at the centre of the domain, $x=\bqty{10000}{\metre}$.
        The blue line, without artificial viscosity and the red line with artificial velocity.}
    \end{subfigure}
    \caption{Time histories for $\nu = \bqty{0.1}{\square\meter\per\second}$}
\end{figure}
The shock wave has a speed of $u_{\it shock} = \bqty{1.4881}{\metre\per\second}$ without and $u_{\it shock} = \bqty{1.5152}{\metre\per\second}$ with artificial viscosity.
%--------------------------------------------------------------------------------
...
\begin{enumerate}
    ...
    \item a temporal computation (with $t_{stop} = \bqty{10800}{\second}$)
\end{enumerate}
%--------------------------------------------------------------------------------
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/weir_bed_water_level.png}
