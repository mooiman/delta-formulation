...
\begin{align}
    u(x^*,0) &= u_1 +
    (u_2 - u_1) \frac{\phi(x^*) }{ \phi(x^*) + \phi(1-x^*)}
\\
...
\begin{symbollist}
    ...
    \item[$u_1$] Boundary condition at left/west boundary of the domain, \bunit{\metre\per\second}.
    \item[$u_2$] Boundary condition at right/east boundary of the domain, \bunit{\metre\per\second}.
\end{symbollist}
...
\begin{align}
    u(x,t) = u_2 + (u_1 - u_2)/
    \left(1 + \exp\left(\frac{(bc_1 - bc_2)}{2\nu}(x-U_{s}t)\right)\right)
\end{align}
with
\begin{symbollist}
    \item[$u_1$] Boundary condition at left/west boundary of the domain, \bunit{\metre\per\second}.
    \item[$u_2$] Boundary condition at right/east boundary of the domain, \bunit{\metre\per\second}.
    \item[$U_{s}$] Velocity of the shock wave, i.e.\ $U_{shock} = (bc_1 + bc_2)/2$, \bunit{\metre\per\second}
    \nomenclature{$U_{s}$}{\si{\metre\per\second}}{Velocity of the shock wave which appears in Burgers equation}
\end{symbollist}
A standing shock wave at $x=0$ will appear if $U_s = 0$, so $u_1 = -u_2$.
An transformation of the $x$-coordinate is applied to get the shock wave in the middle of the domain>
%--------------------------------------------------------------------------------
\paragraph*{Numerical experiment}
The numerical experiment is performed with the following parameters:
\begin{itemize}
    ...
    \item Prescribed initial velocity, linear velocity profile between $u(0,t) = \bqty{1}{\metre\per\second}$ and $x=L_x$ by $u(L_x,t) = \bqty{-1}{\metre\per\second}$.
