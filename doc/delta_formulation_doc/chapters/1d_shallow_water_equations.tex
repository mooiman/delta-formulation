...
with
\begin{align}
    {\mathit g}(t) =\left(\sin\left(\frac{\pi}{16} (t_h - 4)\right)\right)^{0.2}, \qquad t_h = \frac{t}{3600};
\end{align}
...
\begin{align}
    &\left( \frac{1+\alpha_{\mathit{bnd}}}{\Dt} + \theta \frac{u}{\Dx} \right) \Delta c^{n+1,p+1}_{I+1} +
     \left( \frac{1-2\alpha_{\mathit bnd}}{\Dt} - \theta \frac{u}{\Dx} \right) \Delta c^{n+1,p+1}_I +
      \frac{\alpha_{\mathit bnd}}{\Dt}  \Delta c^{n+1,p+1}_{I-1}  =
    \\
    & = - \left\{
          \frac{1+\alpha_{\mathit bnd}}{\Dt} \left( c^{n+1,p}_{I+1} - c^n_{I+1} \right)
        + \frac{1-2\alpha_{\mathit bnd}}{\Dt} \left( c^{n+1,p}_{I} - c^n_{I} \right)
        + \frac{\alpha_{\mathit bnd}}{\Dt} \left( c^{n+1,p}_{I-1}- c^n_{I-1} \right) +
        \right. \\
    & \left. + \frac{u}{\Dx} \left(c^{n+\theta,p}_{I+1} - c^{n+\theta,p}_{I} \right)
        \right\}
\end{align}
where $\alpha_{\mathit bnd} = 2\alpha -\half$ ($\alpha_{\mathit bnd} = -\quart$ when $\alpha = \frac{1}{8}$).
...
The velocity $u>0$ is convected from the left to the right with its own velocity $u\, \bunit{\meter\per\second}$.
The Burgers equation is discretised on the grid
\begin{figure}[H]
    \centering
    \begin{center}
        \resizebox{0.8\textwidth}{!}{
    ...
    \caption[Definition of the grid to solve the 1D-advection equation]{Water body (blue area), finite volumes (green boxes), computational points (open dots), virtual computational points (black dots), boundary points are at $x_{1}$ (inflow/west boundary) and $x_{I+\half}$ (outflow/east boundary).}\label{fig:water_body_fve_bc_at_node_1}
\end{figure}
The finite volume approach for control volume $\Dx_{i+\half}$ of \autoref{eq:1d_burgers} reads:
\begin{align}
    \int^{x_{i+\half}}_{x_{i-\half}} \pdiff{u}{t}\, dx
    + \half\int^{x_{i+\half}}_{x_{i-\half}} \pdiff{u^2}{x} \, dx
    - \int^{x_{i+\half}}_{x_{i-\half}} \pdiff{}{x}\left( \nu\pdiff{c}{x} \right)\, dx &= 0
    \\
    \int^{x_{i+\half}}_{x_{i-\half}} \pdiff{u}{t}\, dx
    + \half \left( \left. u^2 \right|_{i+\half} - \left. u^2 \right|_{i-\half} \right)
    -  \left(
    \left. \nu\pdiff{u}{x}\right|_{i+\half} -
    \left. \nu\pdiff{u}{x}\right|_{i-\half}
    \right) & = 0
\end{align}
%------------------------------------------------------------------------------
\subsection{Discretization interior}
The discretization in \deltaformulation in the interior of domain $\Omega$ and
for equidistant grid and the mass-matrix as given in \autoref{eq:definition_mass_matrix} reads
\begin{align}
    ...
    \nonumber \\*
    &
    + \left\{ \Dx \Dt_{inv} \left( \frac{1}{8}\left( u^{n+1,p}_{i-1} - u^{n}_{i-1} \right) + \frac{6}{8}\left( u^{n+1,p}_{i} - u^{n}_{i} \right) + \frac{1}{8}\left( u^{n+1,p}_{i+1} - u^{n}_{i+1} \right) \right)  \right\} +
    \nonumber \\*
    &\quad  + \half  \left(
    \left( \half (u^{n+\theta, p+1}_{i} + u^{n+\theta,p+1}_{i+1}) \right)^2 -
    \left( \half (u^{n+\theta, p+1}_{i-1} + u^{n+\theta,p+1}_{i}) \right)^2
    \right) +
    \nonumber \\*
    &\quad - \left( \nu_{i+\half} \frac{u^{n+\theta, p+1}_{i+1} - u^{n+\theta, p+1}_{i}}{\Dx} - \nu_{i-\half} \frac{u^{n+\theta, p+1}_{i} - u^{n+\theta, p+1}_{i-1}}{\Dx}\right) = 0
\end{align}
...
\begin{align}
    \Dx & \Dt_{inv}  \left(\frac{1}{8} \Delta u^{n+1,p+1}_{i-1} + \frac{6}{8} \Delta u^{n+1,p+1}_{i} + \frac{1}{8} \Delta u^{n+1,p+1}_{i+1}\right)
    \nonumber \\*
    & \half \theta \left( u^{n+1,p}_{i+\half} \Delta u^{n+1,p+1}_{i} + u^{n+1,p}_{i+\half} \Delta u^{n+1,p+1}_{i+1}
    - \left( u^{n+1,p}_{i-\half}\Delta u^{n+1,p+1}_{i-1} + u^{n+1,p}_{i-\half} \Delta u^{n+1,p+1}_{i}  \right)\right) +
    \nonumber \\*
    & - \theta \frac{\nu_{i+\half}}{\Dx} \Delta u^{n+1, p+1}_{i+1}
    + \theta \left( \frac{\nu_{i+\half}}{\Dx} + \frac{\nu_{i-\half}}{\Dx} \right) \Delta u^{n+1, p+1}_{i} - \theta \frac{\nu_{i-\half}}{\Dx} \Delta u^{n+1, p+1}_{i-1}
    =
    \nonumber \\*
    & = -\left\{ \Dx \Dt_{inv} \left( \frac{1}{8}\left( u^{n+1,p}_{i-1} - u^{n}_{i-1} \right) + \frac{6}{8}\left( u^{n+1,p}_{i} - u^{n}_{i} \right) + \frac{1}{8}\left( u^{n+1,p}_{i+1} - u^{n}_{i+1} \right)\right) + \right.
    \nonumber \\*
    & \qquad + \half \left((u^{n+\theta, p}_{i} + u^{n+\theta,p}_{i+1})^2 -  (u^{n+\theta, p}_{i-1} +u^{n+\theta,p}_{i})^2 \right) +
    \nonumber \\*
    &\qquad - \left. \left( \nu_{i+\half} \frac{u^{n+\theta,p}_{i+1} - u^{n+\theta,p}_{i}}{\Dx} - \nu_{i-\half} \frac{u^{n+\theta,p}_{i} - u^{n+\theta,p}_{i-1}}{\Dx}\right) \right\}
\end{align}

%------------------------------------------------------------------------------
\subsection{Artificial viscosity $\Psi$}
To constant viscosity $\nu$ in the Burgers equation will be add an artificial viscosity $\Psi(x,t)$ which is space and time dependent.
The adjusted Burgers equation (the \textit{'easy'}-problem) reads then
\begin{align}
    \pdiff{u}{t} + \half \pdiff{u^2}{x} - \pdiff{}{x} \left( \left(\nu + \Psi \right) \pdiff{u}{x} \right) = 0. \label{eq:1d_burgers_easy}
\end{align}

An estimation of the artificial viscosity $\Psi$ is computed as follows (
see \autoref{sec:estimate_artificial_viscosity} and
\autoref{eq:c_E}).
\begin{align}
    c_E = c_\psi^2 \frac{\pi}{2} , \qquad  c_{\it error} = c_\Psi.
\end{align}
%
The follow system of equations need to be solved, based on \citet[eq.\ 18]{Borsboom2001}:
\begin{align}
    &\Psi - c_{\it error}\Dx^2\pdiff[2]{\Psi}{x} = c_E \Dx  \int^{\xi_{i+\half}}_{\xi_{i-\half}} \abs{\pdiff[2]{u}{\xi}} \, d\xi
\end{align}

The follow system of equations need to be solved:
\begin{align}
    &\left(\frac{1}{8} - c_{\it error}\right)\Psi_{i-1} +
    \left(\frac{6}{8} + 2 c_{\it error}\right) \Psi_i +
    \left(\frac{1}{8} - c_{\it error}\right)\Psi_{i+1} =
    \\*
    & = c_E \Dx \abs{\pdiff[2]{u}{\xi}}^{n+1,p}_{i}
\end{align}
%\begin{align}
%    &\left(\frac{1}{8} - c_{\it error}\right)\Psi_{i-1} +
%    \left(\frac{6}{8} + 2 c_{\it error}\right) \Psi_i +
...
%    \\*
%    & = c_E  \left[
%    \frac{\Dx}{2} \left( \quart\left( \abs{\pdiff[2]{u}{\xi}}^{n+1,p}_{i-1} + 3 \abs{\pdiff[2]{u}{\xi}}^{n+1,p}_i \right)\right) +
%    \frac{\Dx}{2} \left( \quart\left( 3 \abs{\pdiff[2]{u}{\xi}}^{n+1,p}_{i} + \abs{\pdiff[2]{u}{\xi}}^{n+1,p}_{i+1} \right)\right)
%    \right]
%\end{align}
with
\begin{align}
    \left.\pdiff[2]{u}{\xi}\right|_i = \frac{u_{i-1} - 2 u_i + u_{i+1} }{\Dxi^2}
\end{align}
%------------------------------------------------------------------------------
\subsection{Discretization at boundaries}
An \textbf{essential} boundary condition at the inflow boundary is needed, left side of the domain.
And, at the right side an outflow boundary 'condition' is required for numerical reasons (called  a \textbf{natural} boundary condition), i.e.\ a discretization of the model equation at the outflow boundary.
The natural boundary conditions is fully determined by the outgoing signal and therefor we use the equation of the outgoing signal, i.e.\ \autoref{eq:left_right_going_equations}.
For the 1-D Burgers equation when omitting the viscosity term it reads:
\begin{align}
    \pdiff{u}{t} + \half\pdiff{u^2}{x} - \pdiff{}{x} \left( (\nu + \Psi)  \pdiff{u}{x} \right) = 0. \label{eq:1d_burgers}
\end{align}
%--------------------------------------------------------------------------------
\subsubsection{Inflow boundary}
The \textbf{essential} boundary condition at the inflow boundary (left/west-side) reads:
\begin{align}
    u(0,t) = u_{\it given}(t), \quad t > 0 \qquad \text{(essential boundary)}
\end{align}
The essential boundary condition is supplied at $x_1$ with the following discretization (\autoref{eq:stencil_ess})
\begin{align}
    & w^{\it ess}_0 \theta  \Delta u^{n+1,p+1}_0 +
    w^{\it ess}_1 \theta  \Delta u^{n+1,p+1}_1 +
    w^{\it ess}_2 \theta  \Delta u^{n+1,p+1}_2
    =  u_{\it given}(t) - u^{n+1,p}_{bnd}
\end{align}
with
\begin{align}
    u^{n+1,p}_{bnd} & = w^{\it ess}_0 u^{n+1,p}_0
    + w^{\it ess}_1 u^{n+1,p}_1
    + w^{\it ess}_2 u^{n+1,p}_2,
\end{align}
Similar if an essential boundary condition is given at the right/east-side of the domain.
%
%--------------------------------------------------------------------------------
\subsubsection{Outflow boundary}
The \textbf{natural} is chosen in that way that as less as possible left going spurious numerical waves are generated at the outflow boundary, i.e.\ nearly no reflection.
The natural boundary condition (right/east-side) is supplied at $x_I$ with the discretization constants as determined by \autoref{eq:stencil_nat} and boundary condition \autoref{eq:1d_burgers} which yields:
\begin{align}
    &\left( \frac{w^{\it nat}_0}{\Dt} + \theta \frac{u^{n+1,p}_{I+1}}{\Dx} \right) \Delta u^{n+1,p+1}_{I+1} +
    \left( \frac{w^{\it nat}_1}{\Dt} - \theta \frac{u^{n+1,p}_{I}}{\Dx} \right) \Delta u^{n+1,p+1}_I +
    \frac{w^{\it nat}_2}{\Dt}  \Delta u^{n+1,p+1}_{I-1}  =
    \nonumber \\
    & = - \left\{
    \frac{u^{n+1,p}_{bnd} - u^{n}_{bnd}}{\Dt} +
    \frac{1}{\Dx}
    \frac{u^{n+\theta,p}_{I+1} + u^{n+\theta,p}_{I}}{2}
    \left(u^{n+\theta,p}_{I+1} - u^{n+\theta,p}_{I} \right)
    \right\}
\end{align}
with
\begin{align}
u^{n+1,p}_{bnd} &
= w^{\it nat}_0 u^{n+1,p}_{I+1}
+ w^{\it nat}_1 u^{n+1,p}_{I  }
+ w^{\it nat}_2 u^{n+1,p}_{I-1},
\\
u^{n}_{bnd} &
= w^{\it nat}_0 u^{n}_{I+1}
+ w^{\it nat}_1 u^{n}_{I  }
+ w^{\it nat}_2 u^{n}_{I-1}.
\end{align}
where
\begin{align}
    w^{\it nat}_0 & = (1+\alpha_{\mathit bnd})/2 = \frac{3}{8}
    \\
    w^{\it nat}_1 & = (1 - 2\alpha_{\mathit bnd})/2 = \frac{6}{8}
    \\
    w^{\it nat}_2 & = (\alpha_{\mathit bnd})/2 = -\frac{1}{8}
\end{align}
with $\alpha_{\mathit bnd} = 2\alpha -\half$ ($\alpha_{\mathit bnd} = -\quart$ when $\alpha = \frac{1}{8}$).
...
We start from the following equations, with convection
...
\end{align}
The acceleration due to gravity is assumed to be constant ($g={\mathit{constant}}$).
...
\end{align}
The linearization of the viscosity term ($\nu = {\mathit constant}$) around iteration level $p$ reads (see \autoref{sec:linearisation_viscosity} for a derivation):
\begin{align}
...
The discretization of  boundary \autoref{eq:essential_bc_zeta_1d} at $x=i+\half$ reads
\begin{align}
...
\begin{align}
    ...
    }}\label{eq:essential_bc_q_1d}
\end{align}
a correction term is added, to prevent drifting away of the solution (an integration constant is missing).
The variable $\eps$ has dimension \bunit{\per\second}.
The discretization of  boundary \autoref{eq:essential_bc_q_1d} at $x=i+\half$ reads
\begin{align}
    &\left(\sqrt{gh^{n+\theta,p+1}} - \frac{q^{n+\theta,p+1}}{h^{n+\theta,p+1}} \right) \pdiff{h}{t} + \pdiff{q}{t} =
    \nonumber \\*
...
\begin{align}
    ...
    \frac{\alpha_{\mathit bnd}}{2} \left( h_{i}
    - 2 h_{i+1} + h_{i+2} \right)
    \nonumber \\*
    & = \half \left( 1 + \alpha_{\mathit bnd} \right) h_{i} +
    \half \left( 1 - 2\alpha_{\mathit bnd} \right) h_{i+1} +
    \half \alpha_{\mathit bnd} h_{i+2}\label{eq:h_at_left_boundary}
    \\
    q_{i+\half} & =  \frac{1}{2} \left( q_{i} + q_{i+1}\right)
    + \frac{\alpha_{\mathit bnd}}{2} \left( q_{i}
    - 2 q_{i+1} + q_{i+2} \right)
    \nonumber \\*
    & = \half \left( 1 + \alpha_{\mathit bnd} \right) q_{i} +
    \half \left( 1 - 2\alpha_{\mathit bnd} \right) q_{i+1} +
    \half \alpha_{\mathit bnd} q_{i+2}\label{eq:q_at_left_boundary}
\end{align}
with $\alpha_{\mathit bnd} = 2 \alpha - \half = -\quart$, see \citet{transpeq-analysisdiscretizationinsidedomain_boundaries.mw}.
%------------------------------------------------------------------------------
...
A coefficient $\alpha_{\mathit bnd}$ is placed before that extra term, the optimal value of this coefficient is taken from the analysis in \citet{transpeq-analysisdiscretizationinsidedomain_boundaries.mw}.
\begin{align}
    & \frac{1}{\Dt}\left( \frac{1}{2} \left( h^{n+1,p+1}_{i} + h^{n+1,p+1}_{i+1} \right)
    + \frac{\alpha_{\mathit bnd}}{2} \left( h^{n+1,p+1}_{i} - 2 h^{n+1,p+1}_{i+1} + h^{n+1,p+1}_{i+2}  \right) \right. +
    \nonumber \\*
    & \qquad  - \left. \left(
    \frac{1}{2} \left( h^{n}_{i} + h^{n}_{i+1} \right)
    + \frac{\alpha_{\mathit bnd}}{2}  \left( h^{n}_{i} - 2 h^{n}_{i+1} + h^{n}_{i+2}  \right) \right)
    \right)
\end{align}
...
\begin{align}
    ...
    \nonumber \\*
    & \qquad + \left. \frac{\alpha_{\mathit bnd}}{2}\left( \Delta h^{n+1,p+1}_{i} - 2 \Delta h^{n+1,p+1}_{i+1} + \Delta h^{n+1,p+1}_{i+2} \right) \right) +
    \nonumber \\
    & \qquad + \frac{1}{\Dt} \left\{ \half \left( h^{n+1, p}_{i} + h^{n+1, p}_{i+1} \right)
    + \frac{\alpha_{\mathit bnd}}{2}\left(  h^{n+1,p}_{i} - 2 h^{n+1,p}_{i+1}  + h^{n+1,p}_{i+2} \right) + \right.
    \nonumber \\*
    &
    \qquad \left. - \left( \frac{1}{2} \left( h^{n}_{i} + h^{n}_{i+1} \right)
    + \frac{\alpha_{\mathit bnd}}{2}  \left( h^{n}_{i} - 2 h^{n}_{i+1} + h^{n}_{i+2}  \right) \right) \right\}
\end{align}
...
\begin{align}
    ...
    + \frac{\alpha_{\mathit bnd}}{2} \left( q^{n+1,p+1}_{i} - 2 q^{n+1,p+1}_{i+1} + q^{n+1,p+1}_{i+2}  \right) \right. +
    \nonumber \\*
    & \qquad  - \left. \left(
    \frac{1}{2} \left( q^{n}_{i} + q^{n}_{i+1} \right)
    + \frac{\alpha_{\mathit bnd}}{2}  \left( q^{n}_{i} - 2 q^{n}_{i+1} + q^{n}_{i+2}  \right) \right)
    \right)
\end{align}
After rearranging the equation into an implicit and an explicit part it reads:
\begin{align}
    ...
    \nonumber \\*
    & \qquad + \left. \frac{\alpha_{\mathit bnd}}{2}\left( \Delta q^{n+1,p+1}_{i} - 2 \Delta q^{n+1,p+1}_{i+1} + \Delta q^{n+1,p+1}_{i+2} \right) \right) +
    \nonumber \\
    & \qquad +  \frac{1}{\Dt} \left\{ \half \left( q^{n+1, p}_{i} + q^{n+1, p}_{i+1} \right) + \frac{\alpha_{\mathit bnd}}{2}\left( q^{n+1,p}_{i} - 2 q^{n+1,p}_{i+1}  + q^{n+1,p}_{i+2} \right) + \right.
    \nonumber \\*
    & \qquad
    \left.  - \frac{1}{2} \left( q^{n}_{i} + q^{n}_{i+1} \right) - \frac{\alpha_{\mathit bnd}}{2} \left( q^{n}_{i} - 2 q^{n}_{i+1} + q^{n}_{i+2}  \right) \right\}
\end{align}
%------------------------------------------------------------------------------
\paragraph*{Pressure term}
...
%------------------------------------------------------------------------------
\paragraph*{Bed shear stress term}
...
\end{align}
%------------------------------------------------------------------------------
\paragraph*{Viscosity term}
\textbf{Note:}
Strictly spoken there is no separation between left and right going waves when there is a viscosity term.
%

Notation agreement:
\begin{align}
    \left. \frac{\delta u}{\delta x}\right|_{i+\half} = \frac{u_{i+1} - u_{i}}{\Dx}
\end{align}

At the left/west boundary ($x_{i+\half}$ with $i=0$) the viscosity term for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
%
\begin{align}
    &\pdiff{}{x}\left(\nu h \pdiff{(q/h)}{x}\right) =
    \\
%    &{\color{cyan}\qquad = \pdiff{\nu}{x} h \pdiff{(q/h)}{x} + \nu \pdiff{h}{x} \pdiff{(q/h)}{x} + \nu h \pdiff{}{x}\pdiff{(q/h)}{x} =}
%    \nonumber \\
%    &{\color{black}\qquad =
%        \pdiff{\nu}{x} h \left( \frac{1}{h} \pdiff{q}{x} - \frac{q}{h^2}\pdiff{h}{x}\right)} +
...
%    {\color{blue} \nu \frac{1}{h} \pdiff{h}{x} \left(  \pdiff{q}{x} - \frac{q}{h}\pdiff{h}{x} \right) +}
%    \nonumber \\*
%    &
%    {\color{red} \qquad \phantom{=} + \nu h \Bigg[ -\frac{1}{h^2}  \pdiff{q}{x} \pdiff{h}{x} + \frac{1}{h} \pdiff[2]{q}{x} \quad
%        - \frac{1}{h^2} \pdiff{q}{x}\pdiff{h}{x}
%        + \frac{2q}{h^3} \pdiff{h}{x}\pdiff{h}{x}
%        - \frac{q}{h^2} \pdiff[2]{h}{x}
%        \Bigg]}
%    \nonumber \\*
%    & = \pdiff{\nu}{x} \left(\pdiff{q}{x} - \frac{q}{h}\pdiff{h}{x}\right)
%    {\color{blue} + \cancel{ \nu \frac{1}{h} \pdiff{h}{x}  \pdiff{q}{x}} - \bcancel{\nu \frac{q}{h^2} \pdiff{h}{x} \pdiff{h}{x}}  +}
%    \nonumber \\*
%    & \qquad{\color{red} \phantom{=}
%        \Ccancel[blue]{- \nu \frac{1}{h}  \pdiff{q}{x} \pdiff{h}{x}}
%        + \nu \pdiff[2]{q}{x}
%        - \nu \frac{1}{h} \pdiff{h}{x}\pdiff{q}{x}
%        + \nu \frac{\Cbcancel[blue]{2}q}{h^2} \pdiff{h}{x}\pdiff{h}{x}
...
which will be approximated by
\begin{align}
&\left.  \pdiff{}{x}\left( \nu h \pdiff{(q/h)}{x} \right)  \right|^{n+\theta, p+1}_{i+\half} \approx
\end{align}
\textit{Right hand side}
\begin{align}
&\approx
\frac{\nu_{i+1} - \nu_{i}}{\Dx} \left( \frac{q^{n+\theta,p}_{i+1} - q^{n+\theta,p}_{i}}{\Dx}
- \frac{q^{n+\theta,p}_{i+\half}}{h^{n+\theta,p}_{i+\half}} \frac{h^{n+\theta,p}_{i+1} - h^{n+\theta,p}_{i}}{\Dx} \right) +
\\
&
+ \nu_{i+\half} \frac{ q^{n+\theta,p}_{i} - 2 q^{n+\theta,p}_{i+1} + q^{n+\theta,p}_{i+2} }{\Dx^2}   +
\\
&
- \nu_{i+\half} \frac{1}{h^{n+\theta,p}_{i+\half}} \frac{h^{n+\theta,p}_{i+1} - h^{n+\theta,p}_{i}}{\Dx}
\frac{q^{n+\theta,p}_{i+1} - q^{n+\theta,p}_{i}}{\Dx} +
\\
& + \nu_{i+\half} \frac{q^{n+\theta,p}_{i+\half}}{\left(h^{n+\theta,p}_{i+\half}\right)^2}
\left(\frac{h^{n+\theta,p}_{i+1} - h^{n+\theta,p}_{i}}{\Dx}\right)^2 +
\\
& - \nu_{i+\half} \frac{q^{n+\theta,p}_{i+\half}}{h^{n+\theta,p}_{i+\half}}
\frac{ h^{n+\theta,p}_{i} - 2 h^{n+\theta,p}_{i+1} + h^{n+\theta,p}_{i+2} }{\Dx^2} +
%
\end{align}
\textit{matrix coefficients  for $\Dh$}
\begin{align}
+ \left[ \frac{\delta\nu}{\delta x} \left(
\frac{q^{n+\theta,p}_{i+\half}}{\left(h^{n+\theta,p}_{i+\half}\right)^2} \frac{\delta h}{\delta x}
- \frac{q^{n+\theta,p}_{i+\half}}{h^{n+\theta,p}_{i+\half}} \frac{\delta\cdot}{\delta x}
\right)
\right] & \theta \Dh_{i+\half}+
\\
+ \left[
\nu_{i+\half} \frac{1}{\left(h^{n+\theta,p}_{i+\half}\right)^2} \frac{\delta h}{\delta x} \frac{\delta q}{\delta x}
- \nu_{i+\half} \frac{1}{h^{n+\theta,p}_{i+\half}} \frac{\delta q}{\delta x} \frac{\delta \cdot}{\delta x}
\right] & \theta \Dh_{i+\half}  +
\\
+ \left[
\frac{2q^{n+\theta,p}_{i+\half}}{\left(h^{n+\theta,p}_{i+\half}\right)^2} \frac{\delta h}{\delta x} \frac{\delta \cdot}{\delta x}
-\frac{2q^{n+\theta,p}_{i+\half}}{\left(h^{n+\theta,p}_{i+\half}\right)^3} \frac{\delta h}{\delta x}\frac{\delta \cdot}{\delta x}
\right] & \theta \Dh_{i+\half}  +
 \\
 + \left[
 \frac{q^{n+\theta,p}_{i+\half}}{\left(h^{n+\theta,p}_{i+\half}\right)^2}
 \frac{\delta^2 h}{(\delta x)^2}
 - \frac{q^{n+\theta,p}_{i+\half}}{h^{n+\theta,p}_{i+\half}}
 \frac{\delta^2 \cdot}{(\delta x)^2}
 \right] & \theta \Dh_{i+\half}  +
 \end{align}
\textit{matrix coefficients  for $\Dq$}
\begin{align}
+ \left[
\frac{\delta\nu}{\delta x}  \left( \frac{\delta \cdot}{\delta x}
- \frac{1}{h^{n+\theta,p}_{i+\half}} \frac{\delta h}{\delta x} \right)
\right] & \theta \Dq_{i+\half} +
\\
+ \left[
\nu_{i+\half}\frac{\delta^2\cdot}{(\delta x)^2} \right] & \theta \Dq_{i+\half} +
\\
+ \left[
-\nu_{i+\half} \frac{1}{h^{n+\theta,p}_{i+\half}} \frac{\delta h}{\delta x} \frac{\delta \cdot}{\delta x} \right] & \theta \Dq_{i+\half}
\\
+ \left[
\frac{1}{\left(h^{n+\theta,p}_{i+\half}\right)^2} \frac{\delta h}{\delta x} \frac{\delta h}{\delta x}
\right] & \theta \Dq_{i+\half}  +
\\
+ \left[
 - \frac{1}{h^{n+\theta,p}_{i+\half}}
\frac{\delta^2 h}{(\delta x)^2}
\right] & \theta \Dq_{i+\half}  +
\end{align}
