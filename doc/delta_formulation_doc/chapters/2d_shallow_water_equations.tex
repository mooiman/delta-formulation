...
\begin{align}
    ...
    \label{eq:volume_xieta_continuity_curvi}
    \\*
    ...
    \label{eq:volume_xi_momentum_curvi}
    \\*
    ...
    \label{eq:volume_eta_momentum_curvi}
\end{align}
...
%    - y_{\xi_{qp}}\, \pdiff{\zeta^{n+\theta,p+1}_{qp}}{\eta} \right)\approx \\
 ...
 - y_{\xi_{qp}}\, \pdiff{}{\eta}\left(\zeta^{n+\theta,p}_{qp} + \theta \Delta \zeta_{qp}   \right)
   \right)
\end{align}
with subscript $qp$ the location of the quadrature point in the sub-control volume.
...
The linearization in time for the $q$-momentum equation reads (see \autoref{eq:2d_convection_equation}):
%
\begin{align}
...
\end{align}
The linearization in time for the $r$-momentum equation reads (see \autoref{eq:2d_convection_equation}):
\begin{align}
& \half \Bigg[ \left( y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right)
...
\begin{align}
    ...
    \\
        ...
        \\
        &
        \qquad \qquad - x_{\eta}y_{\eta} \pdiff{(r/h)}{\xi}
        + x_{\eta}y_{\xi} \pdiff{(r/h)}{\eta} +
        \\
        &
        \qquad \qquad - x_{\eta}x_{\eta}\pdiff{(q/h)}{\xi}
        ...
        \\
        &
        \qquad \qquad - x_{\xi}y_{\eta} \pdiff{(r/h)}{\xi}
...
\begin{align}
        ...
        \\
        =& - \frac{1}{J} \pdiff{}{\xi} \Bigg[\frac{\nu h}{J} \bigg( y_{\eta}y_{\eta}  \pdiff{(r/h)}{\xi} - y_{\eta} y_{\xi} \pdiff{(r/h)}{\eta} +
        \\*
        & \qquad \qquad
        + y_{\eta} x_{\eta} \pdiff{(q/h)}{\xi} - y_{\eta} x_{\xi} \pdiff{(q/h)}{\eta} +
        \\*
        & \qquad \qquad
        + 2 x_{\eta}x_{\eta} \pdiff{(r/h)}{\xi} - 2 x_{\xi} x_{\eta} \pdiff{(r/h)}{\eta} \bigg) \Bigg]
        +
        \\
        &+ \frac{1}{J} \pdiff{}{\eta} \Bigg[\frac{\nu h}{J} \bigg( y_{\xi} y_{\eta} \pdiff{(r/h)}{\xi} - y_{\xi} y_{\xi} \pdiff{(r/h)}{\eta}  +
        \\
        & \qquad \qquad
        + y_{\xi} x_{\eta} \pdiff{(q/h)}{\xi} -  y_{\xi} x_{\xi} \pdiff{(q/h)}{\eta} +
        \\
        & \qquad \qquad
        + 2 x_{\eta}x_{\xi} \pdiff{(r/h)}{\xi} - 2 x_{\xi}x_{\xi} \pdiff{(r/h)}{\eta} \bigg) \Bigg]
    \end{align}
%
After applying Green's theorem these terms need to be computed for each of the control volume faces, taken into account the outward normal vector, i.e.\ eight faces on a cartesian/curvilinear grid.

\subsubsection*{Cartesian grid}
On a cartesian grid ($ y_\xi = x_\eta = 0$) these viscous terms reduce for the $x$-momentum equation to:
\begin{align}
    - \Bigg[& \frac{\nu}{J} \bigg(
    ...
    \\
    +\Bigg[& \frac{\nu}{J} \bigg(
    - x_{\xi}y_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
    + x_{\xi}x_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \neta
    \label{eq:cart_xi_viscous_term}
\end{align}
and for the $y$-momentum to:
\begin{align}
    - \Bigg[&\frac{\nu}{J} \bigg(
    y_{\eta} y_{\eta}  \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
    - y_{\eta} x_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right)     \bigg) \Bigg] \nxi
    +
    \\
    + \Bigg[ & \frac{\nu}{J} \bigg(
    - 2 x_{\xi}x_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \neta
...
\begin{figure}[H]
    ...
    \resizebox{0.65\textwidth}{!}{
        \input{figures/curvi_grid_along_straight_boundary_essential_natural_2.pdf_tex}
    }
\end{center}
...
A similar derivation can be given for right/east, and bottom/south and top/north boundary of the computational domain.
...
\begin{figure}[H]
    \begin{center}
        \def\svgwidth{0.80\textwidth} % scaling text
        \resizebox{0.65\textwidth}{!}{
            \input{figures/curvi_grid_along_straight_boundary_essential.pdf_tex}
        }
    \end{center}
...
The discretization of  boundary \autoref{eq:essential_bc_2d_zeta} at $x=i+\half$ reads (when $\lpdiff{z_b}{t}=0$):
\begin{align}
...
%--------------------------------------------------------------------------------
\input{chapters/line_integral_boundary.tex}

%------------------------------------------------------------------------------
\subsection{Discretization boundary}
An extra diffusion term is added to the space boundary implementation, this term is added to the boundary equation to damp the reflection of spurious waves.
A coefficient $\alpha_{\mathit bnd}$ is placed before that extra term, the optimal value of this coefficient is taken from the analysis in \citet{transpeq-analysisdiscretizationinsidedomain_boundaries.mw}, $\alpha_{\mathit bnd} = 2 \alpha -1 = -\quart$.
%
For the $h$ boundary condition at the left/west boundary it reads:
\begin{align}
    h_{i+\half} & = \frac{1}{2} \left( h_{i} +  h_{i+1} \right) +
    \frac{\alpha_{\mathit bnd}}{2} \left( h_{i}
    - 2 h_{i+1} + h_{i+2} \right)
\end{align}
Similar for discharge $q$ and $r$.
After formulation the equation to the \deltaformulation, and rearranging it in an implicit and the explicit part it reads:
\begin{align}
     \frac{1}{\Dt}  \theta &\Bigg[ \half \left( \Delta h^{n+1, p+1}_{i-1, j-\quart} + \Delta h^{n+1, p+1}_{i, j-\quart} \right) +
    \nonumber \\*
    & \quad + \frac{\alpha_{\mathit bnd}}{2} \left( \Delta h^{n+1,p+1}_{i-1, j-\quart} - 2 \Delta h^{n+1,p+1}_{i, j-\quart} + \Delta h^{n+1,p+1}_{i+1, j-\quart} \right) \Bigg] +
    \nonumber \\
    + \frac{1}{\Dt} & \Bigg[ \half \left( h^{n+1, p}_{i-1, j-\quart} + h^{n+1, p}_{i, j-\quart} \right)
    + \frac{\alpha_{\mathit bnd}}{2}\left(  h^{n+1,p}_{i-1, j-\quart} - 2 h^{n+1,p}_{i, j-\quart}  + h^{n+1,p}_{i+1, j-\quart} \right) +
    \nonumber \\*
    &
    \quad  - \left( \frac{1}{2} \left( h^{n}_{i-1, j-\quart} + h^{n}_{i, j-\quart} \right)
    + \frac{\alpha_{\mathit bnd}}{2}  \left( h^{n}_{i-1, j-\quart} - 2 h^{n}_{i, j-\quart} + h^{n}_{i+1, j-\quart}  \right) \right) \Bigg]
\end{align}

%--------------------------------------------------------------------------------
\paragraph*{Time derivative}
At the left/west boundary ($x_{i-\half, j-\quart}$ with $i=1$) the time discretization of the equation reads:
\begin{align}
    \left.\int_\Gamma\pdiff{h}{t}\, dS \right|_{i-\half, j-\quart} & \approx  \half \sqrt{g_{\eta\eta}} \frac{1}{\Dt}   \left(  h^{n+1}_{i-\half, j-\quart} - h^{n}_{i-\half, j-\quart} \right)
\end{align}
with $g_{\eta\eta}$ defined at $(i-\half, j-\quart)$.
Similar discretization is suitable for the discharges $q$ and $r$.

The discretization reads:
%
\begin{align}
    & \half \sqrt{g_{\eta\eta}} \frac{1}{\Dt}\left[ \frac{1}{2} \left( h^{n+1,p+1}_{i-1, j-\quart} + h^{n+1,p+1}_{i, j-\quart} \right)
    + \frac{\alpha_{\mathit bnd}}{2} \left( h^{n+1,p+1}_{i-1, j-\quart} - 2 h^{n+1,p+1}_{i, j-\quart} + h^{n+1,p+1}_{i+1, j-\quart}  \right) \right. +
    \nonumber \\*
    & \qquad  - \left. \left(
    \frac{1}{2} \left( h^{n}_{i-1, j-\quart} + h^{n}_{i, j-\quart} \right)
    + \frac{\alpha_{\mathit bnd}}{2}  \left( h^{n}_{i-1, j-\quart} - 2 h^{n}_{i, j-\quart} + h^{n}_{i+1, j-\quart}  \right) \right)
    \right]
\end{align}
After rearranging the equation to the \deltaformulation, the implicit and the explicit part reads:
\begin{align}
    \half \sqrt{g_{\eta\eta}} \frac{1}{\Dt}  \theta &\Bigg[ \half \left( \Delta h^{n+1, p+1}_{i-1, j-\quart} + \Delta h^{n+1, p+1}_{i, j-\quart} \right) +
    \nonumber \\*
    & \quad + \frac{\alpha_{\mathit bnd}}{2} \left( \Delta h^{n+1,p+1}_{i-1, j-\quart} - 2 \Delta h^{n+1,p+1}_{i, j-\quart} + \Delta h^{n+1,p+1}_{i+1, j-\quart} \right) \Bigg] +
    \nonumber \\
    + \half \sqrt{g_{\eta\eta}} \frac{1}{\Dt} & \Bigg[ \half \left( h^{n+1, p}_{i-1, j-\quart} + h^{n+1, p}_{i, j-\quart} \right)
    + \frac{\alpha_{\mathit bnd}}{2}\left(  h^{n+1,p}_{i-1, j-\quart} - 2 h^{n+1,p}_{i, j-\quart}  + h^{n+1,p}_{i+1, j-\quart} \right) +
    \nonumber \\*
    &
    \quad  - \left( \frac{1}{2} \left( h^{n}_{i-1, j-\quart} + h^{n}_{i, j-\quart} \right)
...
%------------------------------------------------------------------------------
\paragraph*{Mass flux}
At the left/west boundary ($x_{i-\half, j-\quart}$ with $i=1$) the discretization of the mass flux boundary condition reads:
%
\begin{align}
...
\begin{align}
%    \half \sqrt{g_{\eta\eta}} & \frac{1}{J} \bigg[
%    \\
%    & \left( y_{\eta} ( q^{n+\theta, p} + \theta \Delta q) - x_\eta (r^{n+\theta, p} + \theta \Delta r )\right)_{i, j-\quart} +
%    \\
%    - & \left( y_{\eta} ( q^{n+\theta, p} + \theta \Delta q) - x_\eta (r^{n+\theta, p} + \theta \Delta r )\right)_{i-1, j-\quart} +
%    \\
%    + & \left(-y_{\xi} ( q^{n+\theta, p} + \theta \Delta q) + x_\xi (r^{n+\theta, p} + \theta \Delta r )\right)_{i-\half, j} +
%    \\
%    - &\left(-y_{\xi} ( q^{n+\theta, p} + \theta \Delta q) + x_\xi (r^{n+\theta, p} + \theta \Delta r )\right)_{i-\half, j-1}
%    \bigg]
%    \\
%    \Leftrightarrow &
%    \\
    \half \sqrt{g_{\eta\eta}} & \frac{1}{J} \theta \bigg[
    \nonumber \\
    & \left( y_{\eta} \Delta q - x_\eta \Delta r \right)_{i, j-\quart} - \left( y_{\eta} \Delta q - x_\eta \Delta r \right)_{i-1, j-\quart} +
    \nonumber \\
    + & \left(-y_{\xi} \Delta q + x_\xi \Delta r \right)_{i-\half, j} - \left(-y_{\xi} \Delta q + x_\xi \Delta r \right)_{i-\half, j-1}
    \bigg]
    +
    \\
    + \half \sqrt{g_{\eta\eta}} & \frac{1}{J} \bigg[
    \nonumber \\
    & \left( y_{\eta} q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right)_{i, j-\quart}
    -  \left( y_{\eta} q^{n+\theta, p} - x_\eta (r^{n+\theta, p} \right)_{i-1, j-\quart} +
    \nonumber \\
    + & \left(-y_{\xi} ( q^{n+\theta, p} + x_\xi r^{n+\theta, p} \right)_{i-\half, j} - \left(-y_{\xi} q^{n+\theta, p}  + x_\xi (r^{n+\theta, p} \right)_{i-\half, j-1}
    \bigg]
\end{align}
%------------------------------------------------------------------------------
\paragraph*{Pressure term}
At the left/west boundary ($x_{i-\half, j-\quart}$ with $i=1$) the discretization of the pressure term  for the $q$-momentum equation reads:
\begin{align}
    \left. \int_\Gamma\left(gh \pdiff{\zeta}{x}\right)\, dF \right|_{i-\half, j-\quart} \approx
    & \half \sqrt{g_{\eta\eta}}  g h \Jinv \left( y_{\eta}\, \pdiff{\zeta}{\xi} - y_{\xi}\, \pdiff{\zeta}{\eta} \right)
\end{align}
and for the $r$-momentum equation it reads:
\begin{align}
    \left. \int_\Gamma\left(gh \pdiff{\zeta}{y}\right)\, dS \right|_{i-\half, j-\quart} \approx
    & \half \sqrt{g_{\eta\eta}}  g h \Jinv \left( -x_{\eta}\, \pdiff{\zeta}{\xi} + x_{\xi}\, \pdiff{\zeta}{\eta} \right)
\end{align}
...
The pressure term in the $q$-momentum equation will then be approximated by after rearranging the equation into an implicit and a explicit part it reads:
\begin{align}
    & \half \sqrt{g_{\eta\eta}} \theta g \Jinv \bigg[
y_n \left( \zeta^{n+\theta,p}_{i, j-\quart} - \zeta^{n+\theta,p}_{i-1, j-\quart} \right)
    - y_\xi \left( \zeta^{n+\theta,p}_{i-\half, j} - \zeta^{n+\theta,p}_{i-\half, j-1} \right) \bigg] \Delta h^{n+1, p+1}_{i-\half, j-\quart} +
    \nonumber \\
    & \quad +
    \half \sqrt{g_{\eta\eta}}  \theta g h^{n+\theta, p}_{i-\half, j-\quart} \Jinv \bigg[ y_\eta
    \left( \Delta \zeta^{n+1,p+1}_{i, j-\quart}  - \Delta \zeta^{n+1,p+1}_{i-1, j-\quart}\right)
    -y_\xi \left( \Delta \zeta^{n+1,p+1}_{i-\half, j}  - \Delta \zeta^{n+1,p+1}_{i-\half, j-1}  \right) \bigg] +
    \nonumber \\*
    & \quad + \half \sqrt{g_{\eta\eta}} g h^{n+1, p}_{i-\half, j-\quart} \Jinv \bigg[
    y_\eta \left( \zeta^{n+\theta,p}_{i, j-\quart} - \zeta^{n+\theta,p}_{i-1, j-\quart} \right)
    - y_\xi \left( \zeta^{n+\theta,p}_{i-\half, j} - \zeta^{n+\theta,p}_{i-\half, j-1} \right) \bigg]
\end{align}
and the pressure term in the $r$-momentum equation will then be approximated by
\begin{align}
    & \half \sqrt{g_{\eta\eta}} \theta g \Jinv \bigg[
    -x_\eta \left( \zeta^{n+\theta,p}_{i, j-\quart} - \zeta^{n+\theta,p}_{i-1, j-\quart} \right)
    + x_\xi \left( \zeta^{n+\theta,p}_{i-\half, j} - \zeta^{n+\theta,p}_{i-\half, j-1} \right) \bigg] \Delta h^{n+1, p+1}_{i-\half, j-\quart} +
    \nonumber \\
    & \quad +
    \half \sqrt{g_{\eta\eta}}  \theta g h^{n+\theta, p}_{i-\half, j-\quart} \Jinv \bigg[ -x_\eta
    \left( \Delta \zeta^{n+1,p+1}_{i, j-\quart}  - \Delta \zeta^{n+1,p+1}_{i-1, j-\quart}\right)
    +x_\xi \left( \Delta \zeta^{n+1,p+1}_{i-\half, j}  - \Delta \zeta^{n+1,p+1}_{i-\half, j-1}  \right) \bigg] +
    \nonumber \\*
    & \quad + \half \sqrt{g_{\eta\eta}} g h^{n+1, p}_{i-\half, j-\quart} \Jinv \bigg[
    -x_\eta \left( \zeta^{n+\theta,p}_{i, j-\quart} - \zeta^{n+\theta,p}_{i-1, j-\quart} \right)
    +x_\xi \left( \zeta^{n+\theta,p}_{i-\half, j} - \zeta^{n+\theta,p}_{i-\half, j-1} \right) \bigg]
\end{align}
%------------------------------------------------------------------------------
...
\begin{align}
    &
	\half\sqrt{g_{\eta\eta}} \frac{1}{J} \left(y_{\eta} \pdiff{(q^2/h)}{\xi} - y_{\xi} \pdiff{(q^2/h)}{\eta} -x_{\eta} \pdiff{(qr/h)}{\xi} + x_{\xi} \pdiff{(qr/h)}{\eta}\right)
\end{align}
see \autoref{eq:xi_convection_term_1},
and for the $r$-momentum equation, the transformed line integral it reads:
\begin{align}
    \half\sqrt{g_{\eta\eta}}
    \frac{1}{J} \left(y_{\eta} \pdiff{(rq/h)}{\xi} - y_{\xi} \pdiff{(rq/h)}{\eta} -x_{\eta} \pdiff{(r^2/h)}{\xi} + x_{\xi} \pdiff{(r^2/h)}{\eta}\right)
\end{align}
see \autoref{eq:eta_convection_term_1}.

Written in linear terms for the $\xi$-derivative they (similar for the $\eta$-derivative) read:
\begin{align}
    \pdiff{(qq/h)}{\xi} &= \frac{2q}{h}\pdiff{q}{\xi} -\frac{q^2}{h^2} \pdiff{h}{\xi}
    \\*
    \pdiff{(qr/h)}{\xi} &= \frac{r}{h}\pdiff{q}{\xi} + \frac{q}{h}\pdiff{r}{\xi} - \frac{qr}{h^2} \pdiff{h}{\xi}
    \\*
    \pdiff{(rr/h)}{\xi} &= \frac{2r}{h}\pdiff{r}{\xi} -\frac{r^2}{h^2} \pdiff{h}{\xi}
\end{align}
\paragraph*{$q$-momentum equation}
The approximated at the quadrature point $qp$ and at time level ${n+\theta, p}$ by
{\small
\begin{align}
    & \underbrace{ \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
        \frac{2q}{h}
    ...
    & + \underbrace{ \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
    \frac{r}{h}
    \left( -x_\eta \pdiff{q}{\xi} + x_\xi \pdiff{q}{\eta}  \right)
    + \frac{q}{h}
    \left( -x_\eta \pdiff{r}{\xi} + x_\xi \pdiff{r}{\eta}  \right)
    - \frac{qr}{h^2}
    \left(  -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta}  \right)
    \Bigg]}_{\textit{ to right hand side}} +
\end{align}
Jacobian to $h$
\begin{align}
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dh  \underbrace{\Bigg[
...
Jacobian to $q$
\begin{align}
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dq \underbrace{\Bigg[
...
Jacobian to $r$
\begin{align}
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dr \underbrace{\Bigg[
    \frac{1}{h}
    \left( -x_\eta \pdiff{q}{\xi} + x_\xi \pdiff{q}{\eta}  \right)
    - \frac{q}{h^2}
    \left(  -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta}  \right)
    \Bigg]}_{\cal B} +
\end{align}
Gradients of $\Dh$, $\Dq$ and $\Dr$
\begin{align}
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Bigg[ \underbrace{\left(-\frac{q^2}{h^2}\right)}_{\cal C} \left(  y_\eta \pdiff{\Dh}{\xi} - y_\xi \pdiff{\Dh}{\eta} \right)
    +
    \underbrace{\frac{2q}{h}}_{\cal D} \left( y_\eta \pdiff{\Dq}{\xi} - y_\xi \pdiff{\Dq}{\eta} \right)
    \\*
    & + \frac{r}{h}
    \left( -x_\eta \pdiff{\Dq}{\xi} + x_\xi \pdiff{\Dq}{\eta}  \right)
    + \frac{q}{h}
    \left( -x_\eta \pdiff{\Dr}{\xi} + x_\xi \pdiff{\Dr}{\eta}  \right)
    + \left( -\frac{qr}{h^2} \right)
    \left(  -x_\eta \pdiff{\Dh}{\xi} + x_\xi \pdiff{\Dh}{\eta}  \right)
    \Bigg]
\end{align}
}%
%
 and where $\cal{A}$, $\cal{B}$, $\cal{C}$ and $\cal{D}$ multiplied by $\half\sqrt{g_{\eta\eta}} \Jinv \theta$  and gradients supply the coefficients in the matrix of the \deltaformulation,
all these terms evaluated at the quadrature point $qp$.

\paragraph*{$r$-momentum equation}
The approximated at the quadrature point $qp$ and at time level ${n+\theta, p}$ by
{\small
\begin{align}
    & \underbrace{ \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
    ...
    \left( y_\eta \pdiff{q}{\xi} - y_\xi \pdiff{q}{\eta}  \right)
    + \frac{q}{h}
    \left( y_\eta \pdiff{r}{\xi} - y_\xi \pdiff{r}{\eta}  \right)
    - \frac{qr}{h^2}
    \left( y_\eta \pdiff{h}{\xi} - y_\xi \pdiff{h}{\eta}  \right)
    \Bigg]}_{\textit{ to right hand side}} +
    \\*
...
\begin{align}
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dh \underbrace{\Bigg[
        ...
        \Bigg]}_{\cal{A}} +
    \\*
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dh  \underbrace{
    \Bigg[
    -\frac{2r}{h^2}
    \left( -x_\eta \pdiff{r}{\xi} + x_\xi \pdiff{r}{\eta}  \right)
    + \frac{r^2}{h^3}
...
\begin{align}
    ...
    \\*
    & + \half \sqrt{g_{\eta\eta}} \Jinv \theta \Dq \underbrace{\Bigg[
    \frac{2}{h} \left( -x_\eta \pdiff{q}{\xi} + x_\xi \pdiff{q}{\eta} \right)
    - \frac{2r}{h^2} \left( -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta} \right)
    \Bigg]}_{\cal B} +
\end{align}
Gradients of $\Dh$, $\Dq$ and $\Dr$
\begin{align}
    & +  \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
        \frac{r}{h}
        \left( y_\eta \pdiff{\Dq}{\xi} - y_\xi \pdiff{\Dq}{\eta}  \right)
        + \frac{q}{h}
        \left( y_\eta \pdiff{\Dr}{\xi} - y_\xi \pdiff{\Dr}{\eta}  \right)
        - \frac{qr}{h^2}
        \left( y_\eta \pdiff{\Dh}{\xi} - y_\xi \pdiff{\Dh}{\eta}  \right)
        \Bigg] +
    \\*
    & + \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
        \frac{2r}{h}
...
\begin{align}
    ...
    \end{pmatrix}
\end{align}
with $\Psi$ representing the added artificial viscosity due to regularization.

For the $q$-momentum equation the transformed term read (see \autoref{eq:xi_viscous_term}):
\begin{align}
\int_\Gamma \Bigg[
%
- \frac{1}{J} \pdiff{}{\xi}\Bigg[ &  \frac{\nu_{11} h}{J} \bigg(
...
\Bigg] \dotp \vec{ds}
\end{align}
For the $r$-momentum equation the transformed term read (see \autoref{eq:eta_viscous_term}):
\begin{align}
\int_\Gamma \Bigg[
%
