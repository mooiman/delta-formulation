...
\begin{align}
    &\left(q^{n+\theta, p} + \theta \Delta q\right) n_x  + \left(r^{n+\theta, p} + \theta \Delta r\right) n_y
\end{align}
...
For the quadrature point $(i+\half, j+\quart)$ where $\vec{n} = (1,0)^T$ and $\norm{dl} = \half \Dy$ it reads:
\begin{align}
    &\left( q^{n+\theta, p}_{i+\half, j+\quart} + \theta \Delta q_{i+\half, j+\quart} \right) \norm{dl} \approx
    \\*
