...
\begin{align}
    ...
    \\*
    &\quad - \left. \frac{1}{16}\left( h q^{n}_{i, j} +  3 h^{n}_{i+1,j}  + 3  h^{n}_{i, j+1} + h^{n}_{i+1, j+1}\right)\right)
\end{align}
...
\begin{align}
    &\frac{q^{n+\theta, p}q^{n+\theta, p}}{h^{n+\theta, p}} + \frac{q^{n+\theta, p}r^{n+\theta, p}}{h^{n+\theta, p}}
    - \left( \frac{q^{n+\theta, p}q^{n+\theta, p}}{(h^{n+\theta, p})^2} +
    \frac{q^{n+\theta, p}r^{n+\theta, p}}{(h^{n+\theta, p})^2} \right) \theta \Delta h +
    \nonumber \\*
    & + \left ( \frac{2 q^{n+\theta, p}}{h^{n+\theta, p}} +  \frac{r^{n+\theta, p}}{h^{n+\theta, p}}\right) \theta \Delta q + \left( \frac{q^{n+\theta, p}}{h^{n+\theta, p}}  \right) \theta \Delta r
\end{align}
and for the $r$-momentum equation:
\begin{align}
    &\frac{r^{n+\theta, p}q^{n+\theta, p}}{h^{n+\theta, p}} +
    \frac{r^{n+\theta, p}r^{n+\theta, p}}{h^{n+\theta, p}}
- \left( \frac{r^{n+\theta, p}q^{n+\theta, p}}{(h^{n+\theta, p})^2} +
\frac{r^{n+\theta, p}r^{n+\theta, p}}{(h^{n+\theta, p})^2} \right) \theta \Delta h +
\nonumber \\*
& +
\left( \frac{r^{n+\theta, p}}{h^{n+\theta, p}}  \right) \theta \Delta q +
\left (
\frac{q^{n+\theta, p}}{h^{n+\theta, p}} + \frac{2 r^{n+\theta, p}}{h^{n+\theta, p}}  \right) \theta \Delta r
\end{align}
This terms need to be computed for each of the control volume faces, taken into account the outward normal.
Eight faces on a cartesian grid.
...
The linearization in time reads for the $q$-momentum equation (see \autoref{eq:2d_convection_q_equation}):
\begin{align}
        &c_f \left( \frac{q^{n+\theta,p}\abs{\widetilde{\vec{q}^{n+\theta,p}}}}{(h^{n+\theta,p})^2} \right)
        - \left( 2 c_f \frac{ q^{n+\theta,p} \abs{\widetilde{\vec{q}^{n+\theta,p}}} }{(h^{n+\theta,p})^3} \right) \theta \Delta h +
        \\*
        & +
        \left( c_f \frac{\abs{\widetilde{\vec{q}^{n+\theta,p}}}}{(h^{n+\theta,p})^2} + c_f \frac{(q^{n+\theta,p})^4}{(h^{n+\theta,p})^2\abs{\widetilde{\vec{q}^{n+\theta,p}}}^3} \right) \theta \Delta q +
        \left( c_f \frac{q^{n+\theta,p}(r^{n+\theta,p})^3}
        {(h^{n+\theta,p})^2\abs{\widetilde{\vec{q}^{n+\theta,p}}} } \right) \theta \Delta r
 \end{align}
and for the $r$-momentum equation:
\begin{align}
        &c_f \left( \frac{r^{n+\theta,p}\abs{\widetilde{\vec{q}^{n+\theta,p}}}}{(h^{n+\theta,p})^2} \right)
        - \left( 2 c_f \frac{ r^{n+\theta,p} \abs{\widetilde{\vec{q}^{n+\theta,p}}}} {(h^{n+\theta,p})^3} \right) \theta \Delta h +
        \\*
        & + \left( c_f \frac{r^{n+\theta,p}(q^{n+\theta,p})^3}
        {(h^{n+\theta,p})^2\abs{\widetilde{\vec{q}^{n+\theta,p}}} }\right) \theta \Delta q +
        \left( c_f \frac{\abs{\widetilde{\vec{q}^{n+\theta,p}}}}{(h^{n+\theta,p})^2} + c_f \frac{(r^{n+\theta,p})^4}{(h^{n+\theta,p})^2\abs{\widetilde{\vec{q}^{n+\theta,p}}}^3} \right) \theta \Delta r
\end{align}
This terms need to be computed for each of the sub-control volumes.
%%--------------------------------------------------------------------------------
