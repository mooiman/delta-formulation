...
%
%The linearization in time for the $q$-momentum equation reads (see \autoref{eq:2d_convection_q_equation}), $\vecn = (\nxi, \neta)^T = (\pm 1, 0)^T, dl = \half$:
%%
%\begin{align}
%& \half \left\{ y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right\}
%\frac{q^{n+\theta, p}}{h^{n+\theta, p}} \nxi
...
%\end{align}
%
%The linearization in time for the $r$-momentum equation reads (see \autoref{eq:2d_convection_r_equation}), $\vecn = (\nxi, \neta)^T = (0, \pm 1)^T, dl = \half$:
%\begin{align}
%&
...
\begin{align}
    ...
    & = \oint_{\Omega} \nu h
    \left[ 2\pdiff{(q/h)}{x} \nx
    + \left(\pdiff{(q/h)}{y} + \pdiff{(r/h)}{x}\right)\ny \right] \, dl
    \qquad \textit{q-momentum eq.}
    \\
    F_r
    & = \oint_{\Omega}  \nu h
    \left[ \left(\pdiff{(r/h)}{x} + \pdiff{(q/h)}{y}\right)\nx
    + 2\pdiff{(r/h)}{y} \ny \right] \, dl
    \qquad \textit{r-momentum eq.}
\end{align}
Written in linear terms for the derivative yields:
\begin{align}
...
\begin{align}
    ...
    \left[ 2 \nu \left( \pdiff{q}{x} - \frac{q}{h}\pdiff{h}{x}\right)  \nx
    + \left(\nu \left( \pdiff{q}{y} - \frac{q}{h}\pdiff{h}{y} \right) +
    \nu \left( \pdiff{r}{x} - \frac{r}{h}\pdiff{h}{x} \right) \right)\ny \right] \, dl
    \\
    F_r
    & = \oint_{\Omega}
    \left[ \left(\nu \left( \pdiff{r}{x} - \frac{r}{h}\pdiff{h}{x} \right)
    + \nu \left( \pdiff{q}{y} - \frac{q}{h}\pdiff{h}{y} \right) \right)\nx
    + 2 \nu \left( \pdiff{r}{y} - \frac{r}{h}\pdiff{h}{y} \right) \ny \right] \, dl
\end{align}
...
\begin{align}
    ...
    \nonumber \\
    & \qquad + \half \frac{y_\eta}{\Dt} \Bigg[ \half \left( h^{n+1, p}_{i-1, j-\quart} + h^{n+1, p}_{i, j-\quart} \right)
    + \frac{\alpha_{\it bnd}}{2}\left(  h^{n+1,p}_{i-1, j-\quart} - 2 h^{n+1,p}_{i, j-\quart}  + h^{n+1,p}_{i+1, j-\quart} \right) +
    \nonumber \\*
    &
    \qquad - \left( \frac{1}{2} \left( h^{n}_{i-1, j-\quart} + h^{n}_{i, j-\quart} \right)
    + \frac{\alpha_{\it bnd}}{2}  \left( h^{n}_{i-1, j-\quart} - 2 h^{n}_{i, j-\quart} + h^{n}_{i+1, j-\quart}  \right) \right) \Bigg]
\end{align}
%------------------------------------------------------------------------------
...
\begin{align}
    ...
    \half y_\eta \left[ q^{n+\theta, p}_{i, j-\quart} - q^{n+\theta, p+1}_{i-1, j-\quart} \right]
\end{align}
%------------------------------------------------------------------------------
...
\begin{align}
    ...
    = \\
    & =
    \left[ \left( \pdiff{qq/h}{x} + \pdiff{rq/h}{y} \right) \nx +
    \left( \pdiff{qr/h}{x} + \pdiff{rr/h}{y} \right) \ny \right] \norm{ds}
\end{align}
with $\vecn = (\nx, \ny)^T$ the outward normal vector.
...
\end{align}
%
In case we have normal flow at the boundary ($r=0 \Rightarrow \lpdiff{r}{\eta}=0$ and $x_\eta \lpdiff{r}{\xi}=0$) this boundary term reduces to:
\begin{align}
    &\int_{\Gamma_{\xi\eta}} \Bigg[
    \frac{2q}{h}   \left( y_\eta \pdiff{q}{\xi} - y_\xi \pdiff{q}{\eta} \right)
    - \frac{q^2}{h^2}\left( y_\eta \pdiff{h}{\xi} - y_\xi \pdiff{h}{\eta} \right)
\Bigg]\, ds
\end{align}
which will be approximated at each quadrature point $qp$ and at time level ${n+\theta, p}$ by
\begin{align}
    & \underbrace{\half \Bigg[
        \frac{2q}{h}
        \left( y_\eta \pdiff{q}{\xi} - y_\xi \pdiff{q}{\eta}  \right)
        + \left(- \frac{q^2}{h^2}\right)
...
\end{align}
%
And in case we have normal flow at the boundary ($q=0 \Rightarrow \lpdiff{q}{\xi}=0$ and $x_\eta \lpdiff{q}{\eta}=0$) the boundary term reduces to:
\begin{align}
&\int_{\Gamma_{\xi\eta}} \Bigg[
\frac{2r}{h} \left( -x_\eta \pdiff{r}{\xi} + x_\xi \pdiff{r}{\eta} \right)
- \frac{r^2}{h^2}\left( -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta} \right)
...
\begin{align}
    &\underbrace{\half \Bigg[
  \frac{2r}{h}   \left( -x_\eta \pdiff{r}{\xi} + x_\xi \pdiff{r}{\eta} \right)
+ \left(- \frac{r^2}{h^2}\right) \left( -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta} \right) \Bigg] }_{\textit{to right hand side}} +
    \\
&+ \half \theta \Dh \underbrace{\Bigg[
