...
\end{align}and for the $y$-momentum equation the viscous terms can be transformed as (see \autoref{eq:eta_viscous_term}):
\begin{align}
        - &\pdiff{}{x}\left(\nu h\pdiff{(r/h)}{x} + \nu h \pdiff{(q/h)}{y} \right)
...
\begin{align}
    ...
    \nonumber \\
    & \quad +  \half\frac{y_\eta}{\Dt} \left\{ \half \left( q^{n+1, p}_{i-1, j-\quart} + q^{n+1, p}_{i, j-\quart} \right) + \frac{\alpha_{\mathit bnd}}{2}\left( q^{n+1,p}_{i-1, j-\quart} - 2 q^{n+1,p}_{i, j-\quart}  + q^{n+1,p}_{i+1, j-\quart} \right) + \right.
    \nonumber \\*
    & \qquad \qquad
