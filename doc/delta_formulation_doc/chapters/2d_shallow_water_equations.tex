...
Integrating the curvilinear equations over a finite volume $\Omega_{\xi\eta}$ yields:
\begin{align}
...
The Jacobian $J$ is constant in time.
...
\end{align}
where $x_\xi$, $y_\eta$, $y_\xi$ and $x_\eta$ at the quadrature point $qp$ at the centre of the sub-control volume are
\begin{align}
...
The linearization around an iteration step yields:
\begin{align}
...
The discretization of the time derivative term of the momentum equation is similar as  the continuity equation, see \autoref{sec:time_derivative_curvilinear}.
The time derivative for the $q$-momentum equation reads:
\begin{align}
    J \int_{\Omega} \pdiff{q}{t}\, d\omega
\end{align}
and is discretized in the same manner as the continuity equation.
...
x_\xi r^{n+\theta, p} \right) \frac{q^{n+\theta, p}}{h^{n+\theta, p}} n_\eta \Bigg] +
\\
& + \half \theta \Delta h \Bigg[
\left( \frac{- y_\eta (q^{n+\theta, p})^2 + x_\eta {q^{n+\theta, p} r^{n+\theta, p}}}{(h^{n+\theta, p})^2} \right) n_\xi+
\left( \frac{y_\xi (q^{n+\theta, p})^2 - x_\xi q^{n+\theta, p}r^{n+\theta, p} }{(h^{n+\theta, p})^2} \right) n_\eta
\Bigg]+
\\
...
\Bigg]
\end{align}
%with $\vec{n} = (n_\xi, n_\eta)^T = (\pm 1, 0)^T, dl = \half$, it yields:
%\begin{align}
...
+ \left(-y_\xi q^{n+\theta, p} +
x_\xi r^{n+\theta, p} \right) \frac{r^{n+\theta, p}}{h^{n+\theta, p}} n_\eta \Bigg]+
\\
& + \half \theta \Delta h \Bigg[
...
\Bigg] +
\end{align}
%with $\vec{n} = (n_\xi, n_\eta)^T = (0, \pm 1)^T, dl = \half$, it yields:
%\begin{align}
...
%
%The linearization in time for the $q$-momentum equation reads (see \autoref{eq:2d_convection_q_equation}), $\vec{n} = (n_\xi, n_\eta)^T = (\pm 1, 0)^T, dl = \half$:
%%
%\begin{align}
%& \half \left\{ y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right\}
%\frac{q^{n+\theta, p}}{h^{n+\theta, p}} n_{\xi}
...
%\\
%& + \half \theta \Delta h \Bigg[
%-\left\{  y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right\} \frac{q^{n+\theta, p}}{(h^{n+\theta, p})^2} n_\xi
%% + \left( \frac{- x_\xi q^{n+\theta, p}r^{n+\theta, p} + y_\xi (r^{n+\theta, p})^2}{(h^{n+\theta, p})^2} \right) n_\eta
%\Bigg]+
%\\
%& + \half \theta \Delta q \Bigg[
%\left\{y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p}\right\}\frac{1}{h^{n+\theta, p}} +  \frac{y_\eta q^{n+\theta, p}}{h^{n+\theta, p}} n_\xi
%%+ \left( \frac{ - 2y_\xi  q^{n+\theta, p} + x_\xi  r^{n+\theta, p}}{h^{n+\theta, p}} \right) n_\eta
%\Bigg] +
%\\
%& + \half \theta \Delta r \Bigg[
...
%\end{align}
%
%The linearization in time for the $r$-momentum equation reads (see \autoref{eq:2d_convection_r_equation}), $\vec{n} = (n_\xi, n_\eta)^T = (0, \pm 1)^T, dl = \half$:
%\begin{align}
%&
%%\half \left( \left\{ y_\eta q^{n+\theta, p} - x_\eta r^{n+\theta, p} \right\}
%% \frac{r^{n+\theta, p}}{h^{n+\theta, p}} \right) n_{\xi}
%% +
%\half \left(  \left\{ -y_\xi q^{n+\theta, p} +
%x_\xi r^{n+\theta, p} \right\} \frac{r^{n+\theta, p}}{h^{n+\theta, p}} \right) n_\eta +
%\\
%%
%& + \half \theta \Delta h \Bigg[
%%\left( -\frac{ y_\eta q^{n+\theta, p} r^{n+\theta, p} - x_\eta (r^{n+\theta, p})^2}{(h^{n+\theta, p})^2}
...
%& + \half \theta \Delta r \Bigg[
%%\left( \frac{  y_\eta q^{n+\theta, p} - 2 x_\eta r^{n+\theta, p}}{h^{n+\theta, p}} \right) n_\xi+
%\left\{- y_\xi  q^{n+\theta, p} + x_\xi  r^{n+\theta, p} \right\} \frac{1}{h^{n+\theta, p}} + \frac{x_\xi r^{n+\theta, p}}{h^{n+\theta, p}} n_\eta
%\Bigg]
%\end{align}
...
\begin{align}
    J\int_{\Omega_{\xi\eta}} c_f \left( \frac{\vec{q}\abs{\vec{q}}}{h^2} \right)\, d\omega.
\end{align}
The components for the two momentum equations read:
\begin{align}
    F_q &= J\int_{\Omega_{\xi\eta}} c_f \left( \frac{q\abs{\vec{q}}}{h^2} \right)\, d\omega, & \textit{q-momentum eq.}
    \\
    F_r &= J\int_{\Omega_{\xi\eta}} c_f \left( \frac{r\abs{\vec{q}}}{h^2} \right)\, d\omega, & \textit{r-momentum eq.}
\end{align}
...
\begin{align}
    ...
    \nonumber \\
    & \qquad + \half \frac{y_eta}{\Dt} \left\{ \half \left( h^{n+1, p}_{i-1, j-\quart} + h^{n+1, p}_{i, j-\quart} \right)
...
\end{align}
with $\vec{n} = (\texttt{n}_x, n_y)^T$ the outward normal vector.
...
\begin{align}
    ...
    + \frac{q^2}{h^3} \left( y_\eta \pdiff{h}{\xi} - y_\xi \pdiff{h}{\eta} \right)
    \Bigg]}_{\cal A} +
    \\*
    & + \half \theta \Dq \underbrace{\Bigg[
    \frac{2}{h} \left( y_\eta \pdiff{q}{\xi} - y_\xi \pdiff{q}{\eta} \right)
    - \frac{2q}{h} \left( y_\eta \pdiff{h}{\xi} - y_\xi \pdiff{h}{\eta} \right)
    \Bigg]}_{\cal B}
    \nonumber \\*
    & -\half \theta \underbrace{\frac{q^2}{h^2}}_{\cal C} \left(  y_\eta \pdiff{\Dh}{\xi} - y_\xi \pdiff{\Dh}{\eta} \right)
    +
...
+ \frac{r^2}{h^3} \left( -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta} \right)
 \Bigg]}_{\cal A}
\\
& \half \theta \Dr \underbrace{\Bigg[
  \frac{2}{h}  \left( -x_\eta \pdiff{r}{\xi} + x_\xi \pdiff{r}{\eta} \right)
- \frac{2r}{h} \left( -x_\eta \pdiff{h}{\xi} + x_\xi \pdiff{h}{\eta} \right)   \Bigg]}_{\cal B}
\\
& \half \theta  \underbrace{\frac{r^2}{h^2}}_{\cal C} \left( -x_\eta \pdiff{\Dh}{\xi} + x_\xi \pdiff{\Dh}{\eta} \right)
...
\begin{align}
    & \left( \frac{2q}{h}\pdiff{q}{x} - \frac{q^2}{h^2}\pdiff{h}{x}\right)
    =
\\
