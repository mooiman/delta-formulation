...
%
Written in components ($\vec{q} = (q, r)^T$) and for not symmetric viscosity tensor due to the used numerical method
the viscosity term is written as:
\begin{align}
    - \int_{\Omega}
    \begin{pmatrix}
        \pdiff{}{x} \\
    ...
    \, d\omega
\end{align}
For the $q$-momentum equation the viscous terms can be transformed as (see \autoref{sec:viscosity_term_curvilinear}):
%
\begin{align}
    \int_{\Omega} \Bigg\{
    ...
    \Bigg\} \, d\omega
\end{align}
After applying Green's theorem and the outward normal vector defined as  $\vecn = (\nxi, \neta)^T$, yields
\begin{align}
    = \Bigg[ &  \frac{\nu_{11}}{J} \left(
...
\end{align}
and for the $r$-momentum equation the viscous terms can be transformed as (see \autoref{sec:viscosity_term_curvilinear}):
\begin{align}
    \int_{\Omega} \Bigg\{
...
\end{align}
%
These terms need to be computed for each of the control volume faces, taken into account the outward normal vector $\vecn = (\nxi, \neta)^T$, i.e.\ eight faces on a cartesian/curvilinear grid.
...
\end{align}
\paragraph*{$\bm q$-momentum equation}
...
\paragraph*{$\bm r$-momentum equation}
The approximated at the quadrature point $qp$ and at time level ${n+\theta, p}$ by
{\small
\begin{align}
    & \underbrace{ \half \sqrt{g_{\eta\eta}} \Jinv \Bigg[
