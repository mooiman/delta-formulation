...
\begin{align}
    ...
    \, d\omega
\end{align}
For the $x$-momentum equation the viscous terms (just looking to the term within the integral) read:
	\begin{align}
		\begin{aligned}
...
%			- 2 y_{\xi} \pdiff{(y_{\xi}\ q/h)}{\eta} \bigg) \Bigg]
%		\end{aligned}
%	\end{align}
After applying Green’s theorem with
$\vecn = (\nxi, \neta)^T$ the outward normal vector, and multiplied with $J$  the viscosity term of then $x$-momentum equation reads:
\begin{align}
    &- \Bigg[ \frac{\nu h}{J} \bigg(
    2 y_{\eta}  y_{\eta} \pdiff{(q/h)}{\xi}
    - 2 y_{\eta} y_{\xi} \pdiff{(q/h)}{\eta}
    ...
    - x_{\xi}x_{\xi} \pdiff{(q/h)}{\eta} \bigg) \Bigg] \neta
\end{align}
Written with linear functions in the first derivatives and applying Green's theorem yields:\begin{align}
    \Bigg[& \frac{\nu}{J} \bigg(
    -2 y_{\eta}  y_{\eta} \left(\pdiff{q}{\xi} - \frac{q}{h}\pdiff{h}{\xi} \right)
    + 2 y_{\eta} y_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right)
    +
    \nonumber \\*
    &
    + x_{\eta}y_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
    - x_{\eta}y_{\xi}\left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) +
    \nonumber \\*
    &
    - x_{\eta}x_{\eta}\left(\pdiff{q}{\xi} - \frac{q}{h}\pdiff{h}{\xi} \right)
    + x_{\eta}x_{\xi}\left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \nxi +
    \nonumber \\
    +\Bigg[& \frac{\nu}{J} \bigg(
    2 y_{\xi}y_{\eta}  \left(\pdiff{q}{\xi}  - \frac{q}{h}\pdiff{h}{\xi} \right)
    - 2 y_{\xi}y_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right)
    \nonumber \\*
    &
    - x_{\xi}y_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
    + x_{\xi}y_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) +
    \nonumber \\*
    &
    + x_{\xi} x_{\eta} \left(\pdiff{q}{\xi}  - \frac{q}{h}\pdiff{h}{\xi} \right)
...
%        \label{eq:eta_viscous_term}
%	\end{align}
Written with linear functions in the first derivatives and applying Green's theorem yields:
\begin{align}
    \Bigg[&\frac{\nu}{J} \bigg(
      - y_{\eta} y_{\eta}  \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
      + y_{\eta} y_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) +
     \nonumber \\*
     & -  y_{\eta} x_{\eta} \left(\pdiff{q}{\xi} - \frac{q}{h}\pdiff{h}{\xi} \right)
       + y_{\eta} x_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right) +
    \nonumber \\*
    & - 2 x_{\eta}x_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
     + 2 x_{\xi} x_{\eta} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \nxi
    +
    \nonumber \\
    + \Bigg[ & \frac{\nu}{J} \bigg(
       y_{\xi} y_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
     - y_{\xi} y_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) +
    \nonumber \\*
    &
    - y_{\xi} x_{\eta} \left(\pdiff{q}{\xi} - \frac{q}{h}\pdiff{h}{\xi} \right)
    + y_{\xi} x_{\xi} \left(\pdiff{q}{\eta} - \frac{q}{h}\pdiff{h}{\eta} \right) +
    \nonumber \\*
    &  + 2 x_{\eta}x_{\xi} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right) - 2 x_{\xi}x_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \neta \label{eq:eta_greens_viscous_term}
\end{align}

 \subsubsection*{Cartesian grid}
On a cartesian grid ($y_\xi = x_\eta = 0$) these viscous terms  reduce for the $x$-momentum equation (\autoref{eq:xi_greens_viscous_term}) and taken into account the orthotropic form of the viscosity tensor, i.e.:
%
\begin{align}
    \mat{\nu} =
    \begin{pmatrix}
    ...
    \end{pmatrix}
\end{align}
with $\Psi_{11}$ and $\Psi_{22}$ are the added artificial viscosity due to the regularization of the primairy variables (see \autoref{sec:artificial_viscosity}),
%
it yields to
\begin{align}
     \Bigg[& \frac{\nu_{11}}{J} \bigg(
...
\end{align}
and for the $y$-momentum equation (\autoref{eq:eta_greens_viscous_term}) to:
\begin{align}
    \Bigg[&\frac{\nu_{22}}{J} \bigg(
    - y_{\eta} y_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
...
The discretization in time for the main terms inis given and need to performed for each quadrature point at the eight faces of the control volume:
%
\begin{align}
