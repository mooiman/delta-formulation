...
For the $x$-momentum equation the viscous terms (just looking to the term within the integral) read:
	\begin{align}
		...
		\end{aligned}
	\end{align}
and taken into account that the viscous value is not symmetric due to the used numerical method, we write the viscosity value as a tensor:
%
\begin{align}
    \mat{\nu} =
    \begin{pmatrix}
...
The viscous terms for the $x$-equation can be transformed as follows:
\newline
    \textit{step 1}
	\begin{align}
		\begin{aligned}
	...
	\begin{align}
		\begin{aligned}
			=&- \frac{1}{J} \pdiff{}{\xi}\left[ 2\nu_{11} y_{\eta} h\pdiff{(q/h)}{x} - \nu_{12} h x_{\eta}\pdiff{(r/h)}{x} - \nu_{12} h x_{\eta} \pdiff{(q/h)}{y}\right] +
            \\
			& + \frac{1}{J} \pdiff{}{\eta}\left[ 2\nu_{11} y_{\xi} h\pdiff{(q/h)}{x} - \nu_{12} h x_{\xi} \pdiff{(r/h)}{x} - \nu_{12} h x_{\xi} \pdiff{(q/h)}{y}  \right]
		\end{aligned}
	\end{align}
    \textit{step 2}
%    {{
...
\begin{align}
    ...
    - x_{\xi}x_{\xi} \pdiff{(q/h)}{\eta} \bigg) \Bigg] \neta
\end{align}
Written with linear functions in the first derivatives yields:
\begin{align}
    \Bigg[& \frac{\nu_{11}}{J} \bigg(
...
assume $x_{\xi\eta} - x_{\eta\xi} = 0$ and $y_{\xi\eta} - y_{\eta\xi} = 0$
	\begin{align}
		\begin{aligned}
			=& - \frac{1}{J} \pdiff{}{\xi} \left[\nu_{21} h y_{\eta} \pdiff{(r/h)}{x} + \nu_{21} h y_{\eta} \pdiff{(q/h)}{y} - 2 \nu_{22} h x_{\eta} \pdiff{(r/h)}{y} \right] +
            \\
			&+ \frac{1}{J} \pdiff{}{\eta} \left[\nu_{21} h y_{\xi} \pdiff{(r/h)}{x} + \nu_{21} h y_{\xi} \pdiff{(q/h)}{y} - 2 \nu_{22} h x_{\xi} \pdiff{(r/h)}{y} \right] =
		\end{aligned}
	\end{align}
\textit{step 2}
\begin{align}
...
\begin{align}
    \Bigg[ \frac{\nu_{21}}{J} &\bigg(
     ...
     & -  y_{\eta} x_{\eta} \left(\pdiff{q}{\xi} - \frac{q}{h}\pdiff{h}{\xi} \right)
       ...
       \bigg) +
    \nonumber \\*
     \frac{\nu_{22}}{J} &\bigg( - 2 x_{\eta}x_{\eta} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right)
    ...
    \bigg) +
    \nonumber \\*
    \frac{\nu_{22}}{J} & \bigg( 2 x_{\eta}x_{\xi} \left(\pdiff{r}{\xi} - \frac{r}{h}\pdiff{h}{\xi} \right) - 2 x_{\xi}x_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \neta \label{eq:eta_greens_viscous_term}
\end{align}

 \subsubsection*{Cartesian grid}
On a cartesian grid ($y_\xi = x_\eta = 0$) these viscous terms  reduces for the $x$-momentum equation (i.e.\ \autoref{eq:xi_greens_viscous_term}) and taken into account the orthotropic form of the viscosity tensor, to:
%
\begin{align}
    \mat{\nu} =
    \begin{pmatrix}
...
\begin{align}
    ...
    \bigg) \Bigg] \nxi
\end{align}
and for the $y$-momentum equation to:
\begin{align}
     \Bigg[ & \frac{\nu_{22}}{J} \bigg(
    - 2 x_{\xi}x_{\xi} \left(\pdiff{r}{\eta} - \frac{r}{h}\pdiff{h}{\eta} \right) \bigg) \Bigg] \neta
\end{align}
