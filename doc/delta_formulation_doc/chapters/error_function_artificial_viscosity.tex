...
The mentioned error-function $Err_{\psi}$ is derived in this section and is based on \citet[eq.\ 42]{Borsboom2001}.
...
The error function to determine the artificial viscosity $\Psi$ (\autoref{eq:regularization}) is dependent on the velocities.
To reach suitable velocities the square root is taken from the potential and kinetic energy.
\begin{align}
    \mathit{Err}_{\psi} = Err \left( \sqrt{g \hbar} \right) + Err \left( \sqrt{\half \norm{\frac{\vec{q}}{h^2}} } \right)
\end{align}
%--------------------------------------------------------------------------------
\paragraph*{Based on potential energy}
Looking to square root of the potential energy
