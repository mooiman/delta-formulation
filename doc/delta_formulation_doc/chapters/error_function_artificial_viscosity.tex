%--------------------------------------------------------------------------------
\chapter{Error function needed to regularize with artificial viscosity} \label{sec:error_artificial_viscosity}
...
In this section the error function is derived for the 1-dimensional (\autoref{sec:err_1dimensional}) and 2-dimensional  case (\autoref{sec:err_2dimensional}).
The 2D-dimensional case is derived for curvilinear coordinates.

%--------------------------------------------------------------------------------
\section{One dimensional}\label{sec:err_1dimensional}
...
\end{align}
The error function for artificial viscosity  ${\it Err}_\Psi$ reads (comparable with \citet[eq.\ 42]{Borsboom2001}, differs by a factor of $1/8$):
\begin{align}
...
\begin{align}
    ...
    = 0
\end{align}
the energy equation reads (\label{sec:obtaining_2denergy_equation}):
\begin{align}
    ...
    \right] = 0
\end{align}
When a steady state solution is assumed ($\lpdiff{}{t}=0$, $q=\textit{Constant}$ and $r=\textit{Constant}$) it yields:
\begin{align}
...
\end{align}
%--------------------------------------------------------------------------------
\subsection{Err function}
%--------------------------------------------------------------------------------
\paragraph*{Based on potential energy}
Looking to square root of the potential energy
\begin{align}
    {\it Err} & \left( \sqrt{g \widehat{h}} \right) = \sqrt{g \widehat{h}}
    - \sqrt{g \overline{h}}
    \\*
    & \approx \sqrt{g
        \left(\hbar +
...
%
\begin{align}
    {\it Err}&\left(\sqrt{g\hhat}\right)  = \sqrt{\frac{g}{\hbar}} \left(  \frac{1}{16} F_1(\hbar) +  \frac{1}{8} F_2(\hbar) + \frac{1}{16} F_3(\hbar) \right)
\end{align}

%--------------------------------------------------------------------------------
\paragraph*{Based on kinetic energy}
Looking to square root of the kinetic energy ($\sqrt{\norm{\vec{u}}/2} = \sqrt{\half(q^2/h^2 + r^2/h^2)}$):
\begin{align}
    {\it Err}&\left( \sqrt{\half \left( \frac{\widehat{q}^2}{\widehat{h}^2} + \frac{\widehat{r}^2}{\widehat{h}^2} \right)} \right) =
    \\*
    & =
    \half \sqrt{2} \sqrt{ \left( \frac{\widehat{q}^2}{\widehat{h}^2} + \frac{\widehat{r}^2}{\widehat{h}^2} \right)}
    -
    \half \sqrt{2} \sqrt{\left( \frac{\overline{q}^2}{\overline{h}^2} + \frac{\overline{r}^2}{\overline{h}^2} \right)} \leq
    \\*
    &\text{using $\sqrt{a^2 + b^2} \leq a + b$ when $a>0$ and $b>0$} \nonumber
    \\*
    & \leq
    \half \sqrt{2} \left( \frac{\widehat{q}}{\widehat{h}}
    + \frac{\widehat{r}}{\widehat{h}} \right)
    -
    \half \sqrt{2} \left( \frac{\overline{q}}{\overline{h}}
    + \frac{\overline{r}}{\overline{h}} \right) =
    \\*
    & =
    \half \sqrt{2} \left( \frac{\widehat{q}}{\widehat{h}}
    ...
    - \frac{\overline{r}}{\overline{h}} \right) =
    \\*
    = &{\it Err} \left( \sqrt{\half \frac{\widehat{q}^2}{\widehat{h}^2}} \right)
    +
    {\it Err}\left( \sqrt{\half \frac{\widehat{r}^2}{\widehat{h}^2}} \right)
\end{align}
thus
\begin{align}
    {\it Err}&\left( \sqrt{\half \left( \frac{\widehat{q}^2}{\widehat{h}^2} + \frac{\widehat{r}^2}{\widehat{h}^2} \right)} \right)
...
\end{align}
%
Looking first to the dependencies of $q/h$ in two dimensions, for $r/h$ it is similar:
\begin{align}
    {\it Err}& \left( \sqrt{\half \frac{\widehat{q}^2}{\widehat{h}^2}} \right) =
    \half \sqrt{2} \left( \frac{\widehat{q}}{\widehat{h}}
    - \frac{\overline{q}}{\overline{h}} \right)
...
\end{align}
Because $\lpdiff[2]{z_b}{x}$ is negligible, due to the regularization of the bedlevel, written in curvilinear coordinates when using \autoref{eq:d2hdx2}, \eqref{eq:d2hdxdy} and \eqref{eq:d2hdy2}, we get:
%
\begin{align}
    {\it Err}&\left( \sqrt{\half \frac{\widehat{q}^2}{\widehat{h}^2}} \right) =
    \half \sqrt{2} \left( \frac{\widehat{q}}{\widehat{h}}
...
Applying these multiplications and skipping th convection terms)
\begin{align}
