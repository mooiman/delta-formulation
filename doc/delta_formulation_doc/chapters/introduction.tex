...
This document is based on \citet{Borsboom1998}, where the mathematical model is called the "\textsl{difficult problem}".
...
In \autoref{sec:1d_numerical_outflow_boundary_layer_experiment}: \nameref{sec:1d_numerical_outflow_boundary_layer_experiment}, the fully implicit time integration is presented for the advection-diffusion equation.
Showing a flow from left to right with a Dirichlet boundary condition at the outflow side of the domain.
The Dirichlet value at the outflow boundary is so chosen that there will appear an outflow boundary layer.

In \autoref{sec:1d_numerical_interface_experiment}: \nameref{sec:1d_numerical_interface_experiment}, the fully implicit time integration is presented for the advection-diffusion equation.
...
%            \item Smoothed piecewise linear function $\uhat$, where the support points from $\ubar$ and $\uhat$ are equal.
%        \end{itemize}
%        % \printrefsegment
%}}
