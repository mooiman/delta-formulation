...
%------------------------------------------------------------------------------
\subsection{1-D Advection-diffusion equation}\label{sec:1d_adv_diff_equation}
...
\begin{align}
    ...
    \nonumber \\*
    & +  u \left( \half (c^{n+\theta, p+1}_{i} + c^{n+\theta,p+1}_{i+1}) -  \half (c^{n+\theta, p+1}_{i-1} +c^{n+\theta,p+1}_{i}) \right) +
    \nonumber \\*
    &- \left( \eps_{i+\half} \frac{c^{n+\theta}_{i+1} - c^{n+\theta}_{i}}{\Dx} - \eps_{i-\half} \frac{c^{n+\theta}_{i} - c^{n+\theta}_{i-1}}{\Dx}\right) =
    \nonumber \\*
...
\begin{align}
    ...
    & = -\left\{ \Dx \Dt_{inv} \left( \frac{1}{8}\left( c^{n+1,p}_{i-1} - c^{n}_{i-1} \right) + \frac{6}{8}\left( c^{n+1,p}_{i} - c^{n}_{i} \right) + \frac{1}{8}\left( c^{n+1,p}_{i+1} - c^{n}_{i+1} \right)\right) + \right.
    \nonumber \\*
    & \qquad + \left. \half u \left((c^{n+\theta, p}_{i} + c^{n+\theta,p}_{i+1}) -  (c^{n+\theta, p}_{i-1} +c^{n+\theta,p}_{i}) \right) \right\}
\end{align}
\todo{Viscosity term}
