...
For the place discretizations for an arbitrary function $u$, the following interpolations are used:
\begin{align}
    u_{i+\half} & = \half \left( u_{i+1} + u_{i} \right) &\textit{at interface of sub-control volume}
    \\
    u_{i+\quart} & = \quart \left( 3 u_{i} + u_{i+1} \right) &\textit{at quadrature point of sub-control volume}
    \\
    u_{i-\quart} & = \quart \left( 3 u_{i} + u_{i-1} \right) &\textit{at quadrature point of sub-control volume}
\end{align}
...
\begin{align}
    & \int^{i+\half}_{i_-\half} \pdiff{h}{t}\, dx
\end{align}
...
\begin{align}
    \int_{i-\half}^{i+\half} \pdiff{q}{x} \,dx
\end{align}
...
\begin{align}
    & \int^{i+\half}_{i_-\half} \pdiff{q}{t}\, dx
\end{align}
which will be approximated by
\begin{align}
...
\end{align}
The linearization of the viscosity term ($\nu = {\it constant}$) around iteration level $p$ read (see \autoref{sec:linearisation_viscosity} for a derivation):
\begin{align}
...
\begin{align}
\left(\sqrt{gh} - \frac{q}{h}\right) \underbrace{ \left(\pdiff{h}{t} + \pdiff{q}{x} + \ldots \right) }_{\text{continuity eq.}} - \underbrace{\pdiff{q}{t} + g h \pdiff{\zeta}{x}+\ldots}_{\text{momentum eq.}} = 0
\end{align}
For the \textbf{natural} boundary the values at the boundary of $h$ and $q$ are computed as follows:
%
\begin{align}
...
At the left/west boundary the time discretization of the continuity equation for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
...
\begin{align}
    ...
    \nonumber \\*
    & \qquad
    \left. - \frac{1}{2} \left( h^{n}_{i} + h^{n}_{i+1} \right)   - \frac{\alpha_{bc}}{2}  \left( h^{n}_{i} - 2 h^{n}_{i+1} + h^{n}_{i+2}  \right) \right) + \ldots
\end{align}

%------------------------------------------------------------------------------
\paragraph*{Mass flux, continuity equation}
At the left/west boundary the discretization of the mass flux for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
...
At the left/west boundary the time discretization of the momentum equation for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
...
\begin{align}
    ...
    \nonumber \\
    & = - \left\{ \frac{1}{\Dt} \left( \half \left( q^{n+1, p}_{i} + q^{n+1, p}_{i+1} \right) + \frac{\alpha_{bc}}{2}\left( q^{n+1,p}_{i} - 2 q^{n+1,p}_{i+1}  + q^{n+1,p}_{i+2} \right) + \right. \right.
    \nonumber \\*
    & \qquad
    \left. \left. - \frac{1}{2} \left( q^{n}_{i} + q^{n}_{i+1} \right) - \frac{\alpha_{bc}}{2} \left( q^{n}_{i} - 2 q^{n}_{i+1} + q^{n}_{i+2}  \right) \right)\right\} + \ldots
\end{align}
%------------------------------------------------------------------------------
\paragraph*{Pressure term, momentum equation}
At the left/west boundary the discretization of the pressure term for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
...
\theta \left( \Delta \zeta^{n+1,p+1}_{i+1}  - \Delta \zeta^{n+1,p+1}_{i}\right) = \ldots
\end{align}
After rearranging the equation into an implicit left hand side and an explicit right hand side  it reads:
\begin{align}
...
\nonumber \\*
&= -\left\{
\frac{1}{\Dx} g h^{n+\theta,p}_{i+\half} \left( \zeta^{n+\theta,p}_{i+1} - \zeta^{n+\theta,p}_{i}  \right)  \right\} + \ldots
\end{align}
%------------------------------------------------------------------------------
\paragraph*{Convection term, momentum equation}
At the left/west boundary the discretization of the convection term for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
...
At the left/west boundary the discretization of the bed shear stress term for the \textbf{natural} boundary condition, describing the outgoing wave, reads:
\begin{align}
