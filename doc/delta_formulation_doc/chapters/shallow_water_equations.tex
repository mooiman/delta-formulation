...
after rearranging the equation into an implicit left hand side and an explicit right hand side  it reads:
\begin{align}
\half  \theta \Delta q^{n+1, p+1}_{i} - \half \theta \Delta q^{n+1, p+1}_{i-1} = -\left\{ \half q^{n+1,p}_{i+1}  - \half q^{n+1,p}_{i-1}     \right\}
\end{align}
...
\begin{align}
    ...
    \int_{x_{i}}^{x_{i+\half}} c_f\frac{q\abs{q}}{h^2} \,dx
    \\
