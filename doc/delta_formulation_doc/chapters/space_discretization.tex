...
\end{figure}
%------------------------------------------------------------------------------
\section{Compatible initialization}\label{sec:compatible_initialization}
...
\begin{figure}[H]
    ...
    \caption[Illustration of compatible quadratic function]{Illustration of compatible quadratic function. The integral over the control volume of the the given function (blue) is equal represented by the red-function and better then by the green-function, therefor the red function is called compatible with the given function. In green-function is defined with nodes on the given function. Control volumes shown by the green vertical lines. And the gray areas are virtual and are not belonging to the water body (see \autoref{fig:water_body_fve_bc_at_node}). }\label{fig:compatible_initial_condition}
