...
\end{align}
The parabolic fit that gives neither under- nor over-specification of
...
\end{align}
the weight coefficient before $c_0$, $c_1$ and $c_2$ are called $w^{\it ess}_0$, $w^{\it ess}_1$ and $w^{\it ess}_2$.
...
\begin{align}
\int_{{\half}}^{{\frac{3}{2}}} c_{opt}(\xi)\, d\xi = \frac{1}{8} c_0 + \frac{6}{8} c_1 + \frac{1}{8} c_2
\end{align}
...
\end{align}
For the Weir experiment (\autoref{sec:numerical_experiment_weir}) the following parameters are used, $\alpha_{\psi} = c_{\Psi}$ and $\beta_{\psi} \approx 7 c_{\Psi}$,
see \autoref{tab:numerical_accuracy_c_error}.
...
%--------------------------------------------------------------------------------
\subsection{Determination of artificial smoothing coefficient $\mathbf \Psi$}
\label{sec:determine_psi}
The artificial smoothing coefficient $\Psi$ ($ = c_{\psi} \Dx^2 E$, \autoref{eq:regularization}) is dependent on error $E$, a smoothing coefficent $c_E$ and the second derivative of the given function $u_{giv}$ \citep[eq.\ 8]{Borsboom1998}.
...
\begin{align}
    D_i & = \Dxi^2 \pdiff[2]{\uhat}{\xi} \quad \text{\citep[eq.\ 2]{Borsboom1998}}
    \\ & \approx {\ubar}_{i-1} - 2 {\ubar}_{i-1} + {\ubar}_{i+1}
\end{align}
%
Now the system of \autoref{eq:error_regularization} can be solved and where $\vec{\Psi}$ is set to:
\begin{align}
...
\end{figure}
%------------------------------------------------------------------------------
\section{Compatible initialization in one dimension}\label{sec:compatible_initialization_1d}
...
\begin{align}
    \int_{x_{i-\half}}^{x_{i+\half}} \overline u_{initial}\, dx = \int_{x_{i-\half}}^{x_{i+\half}} u_{given}\, dx \label{eq:compatible_initialization_1d}
\end{align}
...
An illustration off the result of \autoref{eq:compatible_initialization_1d} is given in \autoref{fig:compatible_initial_condition}.
\begin{figure}[H]
    \centering
...
\end{align}
%------------------------------------------------------------------------------
\section{Compatible initialization in two dimensions}\label{sec:compatible_initialization_2d}
...
\begin{align}
    \int_{y_{i-\half}}^{x_{i+\half}} \int_{y_{i-\half}}^{x_{i+\half}} \overline u_{initial}\, dxdy = \int_{y_{i-\half}}^{y_{i+\half}} \int_{x_{i-\half}}^{x_{i+\half}} u_{given}\, dxdy \label{eq:compatible_initialization_2d}
\end{align}
where $\overline{u}$ is the piecewise linear function and $u_{given}$ the given initial condition.
