...
void adv_diff_init_concentration(std::vector<double>& mass, std::vector<double> & x, double Lx, double c_init, std::string shape, std::vector<double>& c_out)
{
    double L_envelope;
    double fcent;
    double shift;
    ...
    std::vector<double> c_ana(refine * (nx - 1) + 1, 0.0);
    ...
    else if (shape == "envelope")
    {
        ...
        for (int i = 1; i < refine * (nx - 1) + 1; ++i)
        {
            ...
            {
                c_ana[i] = (0.5 + 0.5 * cos(2.0 * M_PI * 5. * (x_ana[i] - fcent) / L_envelope)) *
                    (0.5 + 0.5 * cos(2.0 * M_PI * 1. * (x_ana[i] - fcent) / L_envelope));
            }
        }
        (void) control_volumes(c_ana, cv, dx, refine);
        (void) compatible_function(mass, cv, c_ana, c_out, dx, refine);
    }
    else if (shape == "envelope_phi")
    {
        // Special function, as supplied by Mart Borsboom
        L_envelope = 0.25 * Lx;
        shift = 0.125 * Lx;
        ...
        for (int i = 0; i < nx; ++i)
        {
            if (c_out[i] < 1.e-12)
            {
                c_out[i] = -25.;  // $ln(1e-12) \pm -27.0$
            }
            else
            {
                c_out[i] = std::log(c_out[i]);
            }
        }
    }
}
...
void compatible_function(std::vector<double>& mass, std::vector<double>& cv, std::vector<double>& c_ana, std::vector<double>& c_out, 
    double dx, size_t refine)
{
    size_t nx = cv.size();
    size_t nx_ana = c_ana.size();
