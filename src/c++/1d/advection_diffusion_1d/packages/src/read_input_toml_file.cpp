...
_data_input read_toml_file(std::filesystem::path & input_dir, std::filesystem::path & toml_file_name){
    ...
    data.initial.c_initial = tbl_chp["c_initial"].value_or(double(0.0));  
    status = get_toml_array(tbl_chp, "ini_vars", data.initial.ini_vars);
    ...
    data.physics.u_advection = tbl_chp["u_advection"].value_or(double(0.0));  
    data.physics.do_advection = tbl_chp["do_advection"].value_or(bool(false));
    ...
    data.physics.visc_shape = tbl_chp["visc_shape"].value_or("constant");
