...
void initial_conditions(std::vector<double>& x, size_t nx,
    ...
    double amp, double gauss_mu_x, double gauss_sigma_x)
{
    // 
    ...
    for (size_t var_i = 0; var_i < ini_vars.size(); var_i ++)
    {
        for (size_t i = 0; i < nx; i++)
        {
            ...
            {
                s[k] = ini_vals[var_i];  // initial water level
            }
            else if (ini_vars[var_i] == "zeta_linear_x")
            {
                size_t k0 = 0;
                size_t k1 = nx - 1;
                s[k] = ini_vals[var_i] * (x[k] - x[0]) / (x[k1] - x[k0]); // initial water level
            }
            else if (ini_vars[var_i] == "zeta_gauss_hump")
            {
                s[k] = amp * std::exp( -(x[k] - gauss_mu_x) * (x[k] - gauss_mu_x) / (2. * gauss_sigma_x * gauss_sigma_x) );  // initial water level
            }
            ...
            {
                u[k] = ini_vals[var_i];  // initial water level
            }
            else
            {
                std::cout << "----------------------------" << std::endl;
                std::cout << "Initialization variables." << std::endl;
                std::cout << "Option: \"" << ini_vars[var_i] << "\" is not supported." << std::endl;
