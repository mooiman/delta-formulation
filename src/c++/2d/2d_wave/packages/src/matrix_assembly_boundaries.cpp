    ...
    std::vector<double>& w_nat, std::vector<double>& w_ess)
{
    ...
    else
    {
        ...
        // face 0
        double face_fac = 0.5 * dx * 0.25;
        add_value(values, col_wb , face_fac * 1.0 * w_nat[0] * theta * g * dzetady_e +  face_fac * 1.0 * dyinv * theta * g * htheta_e);
        add_value(values, col_ws , face_fac * 1.0 * w_nat[1] * theta * g * dzetady_e -  face_fac * 1.0 * dyinv * theta * g * htheta_e);
        add_value(values, col_wss, face_fac * 1.0 * w_nat[2] * theta * g * dzetady_e);
        add_value(values, col_b  , face_fac * 3.0 * w_nat[0] * theta * g * dzetady_b +  face_fac * 3.0 * dyinv * theta * g * htheta_b);
        add_value(values, col_s  , face_fac * 3.0 * w_nat[1] * theta * g * dzetady_b -  face_fac * 3.0 * dyinv * theta * g * htheta_b);
        ...
        add_value(values, col_s  , face_fac * 3.0 * w_nat[1] * theta * g * dzetady_b -  face_fac * 3.0 * dyinv * theta * g * htheta_b);
        add_value(values, col_ss , face_fac * 3.0 * w_nat[2] * theta * g * dzetady_b);
        add_value(values, col_eb , face_fac * 1.0 * w_nat[0] * theta * g * dzetady_w +  face_fac * 1.0 * dyinv * theta * g * htheta_w);
        add_value(values, col_es , face_fac * 1.0 * w_nat[1] * theta * g * dzetady_w -  face_fac * 1.0 * dyinv * theta * g * htheta_w);
        ...
        if (do_convection)
        {
            ...
            // face 0
            double face_fac = 0.5 * dx * 0.25;
            add_value(values, col_wb , face_fac * 1.0 * (theta * aa_w * w_nat[0] + dyinv * theta * cc_w));
            add_value(values, col_ws , face_fac * 1.0 * (theta * aa_w * w_nat[1] - dyinv * theta * cc_w));
            add_value(values, col_wss, face_fac * 1.0 * (theta * aa_w * w_nat[2]));
            add_value(values, col_b  , face_fac * 3.0 * (theta * aa_b * w_nat[0] + dyinv * theta * cc_b));
            add_value(values, col_s  , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dyinv * theta * cc_b));
            ...
            add_value(values, col_s  , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dyinv * theta * cc_b));
            add_value(values, col_ss , face_fac * 3.0 * (theta * aa_b * w_nat[2]));
            add_value(values, col_eb , face_fac * 1.0 * (theta * aa_e * w_nat[0] + dyinv * theta * cc_e));
            add_value(values, col_es , face_fac * 1.0 * (theta * aa_e * w_nat[1] - dyinv * theta * cc_e));
            ...
            // face 0
            face_fac = 0.5 * dx * 0.25;
            add_value(values, col_wb  + 2, face_fac * 1.0 * (theta * bb_w * w_nat[0] + dyinv * theta * dd_w));
            add_value(values, col_ws  + 2, face_fac * 1.0 * (theta * bb_w * w_nat[1] - dyinv * theta * dd_w));
            add_value(values, col_wss + 2, face_fac * 1.0 * (theta * bb_w * w_nat[2]));
            add_value(values, col_b   + 2, face_fac * 3.0 * (theta * bb_b * w_nat[0] + dyinv * theta * dd_b));
            add_value(values, col_s   + 2, face_fac * 3.0 * (theta * bb_b * w_nat[1] - dyinv * theta * dd_b));
            ...
            add_value(values, col_s   + 2, face_fac * 3.0 * (theta * bb_b * w_nat[1] - dyinv * theta * dd_b));
            add_value(values, col_ss  + 2, face_fac * 3.0 * (theta * bb_b * w_nat[2]));
            add_value(values, col_eb  + 2, face_fac * 1.0 * (theta * bb_e * w_nat[0] + dyinv * theta * dd_e));
            add_value(values, col_es  + 2, face_fac * 1.0 * (theta * bb_e * w_nat[1] - dyinv * theta * dd_e));
        ...
        // face 0
        face_fac = 0.5 * dx * 0.25;
        add_value(values, col_wb  + 2, face_fac * 1.0 * con_fac *  dyinv * theta);
        add_value(values, col_ws  + 2, face_fac * 1.0 * con_fac * -dyinv * theta);
        add_value(values, col_wss + 2, face_fac * 1.0 * 0.0);
        add_value(values, col_b   + 2, face_fac * 3.0 * con_fac *  dyinv * theta);
        add_value(values, col_s   + 2, face_fac * 3.0 * con_fac * -dyinv * theta);
        ...
        add_value(values, col_s   + 2, face_fac * 3.0 * con_fac * -dyinv * theta);
        add_value(values, col_ss  + 2, face_fac * 3.0 * 0.0);
        add_value(values, col_eb  + 2, face_fac * 1.0 * con_fac *  dyinv * theta);
        add_value(values, col_es  + 2, face_fac * 1.0 * con_fac * -dyinv * theta);
    ...
    std::vector<double>& w_nat, std::vector<double>& w_ess)
{
    ...
    else
    {
        ...
        // face 0
        double face_fac = 0.5 * dy * 0.25;
        add_value(values, col_nb , face_fac * 1.0 * w_nat[0] * theta * g * dzetadx_n + face_fac * 1.0 * dxinv * theta * g * htheta_n);
        add_value(values, col_nw , face_fac * 1.0 * w_nat[1] * theta * g * dzetadx_n - face_fac * 1.0 * dxinv * theta * g * htheta_n);
        add_value(values, col_nww, face_fac * 1.0 * w_nat[2] * theta * g * dzetadx_n);
        add_value(values, col_b  , face_fac * 3.0 * w_nat[0] * theta * g * dzetadx_b + face_fac * 3.0 * dxinv * theta * g * htheta_b);
        add_value(values, col_w  , face_fac * 3.0 * w_nat[1] * theta * g * dzetadx_b - face_fac * 3.0 * dxinv * theta * g * htheta_b);
        ...
        add_value(values, col_w  , face_fac * 3.0 * w_nat[1] * theta * g * dzetadx_b - face_fac * 3.0 * dxinv * theta * g * htheta_b);
        add_value(values, col_ww , face_fac * 3.0 * w_nat[2] * theta * g * dzetadx_b);
        add_value(values, col_sb , face_fac * 1.0 * w_nat[0] * theta * g * dzetadx_s + face_fac * 1.0 * dxinv * theta * g * htheta_s);
        add_value(values, col_sw , face_fac * 1.0 * w_nat[1] * theta * g * dzetadx_s - face_fac * 1.0 * dxinv * theta * g * htheta_s);
        ...
        if (do_convection)
        {
            ...
            // face 0
            double face_fac = 0.5 * dy * 0.25;
            add_value(values, col_nb , face_fac * 1.0 * (theta * aa_n * w_nat[0] + dxinv * theta * cc_n));
            add_value(values, col_nw , face_fac * 1.0 * (theta * aa_n * w_nat[1] - dxinv * theta * cc_n));
            add_value(values, col_nww, face_fac * 1.0 * (theta * aa_n * w_nat[2]));
            add_value(values, col_b  , face_fac * 3.0 * (theta * aa_b * w_nat[0] + dxinv * theta * cc_b));
            add_value(values, col_w  , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dxinv * theta * cc_b));
            ...
            add_value(values, col_w  , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dxinv * theta * cc_b));
            add_value(values, col_ww , face_fac * 3.0 * (theta * aa_b * w_nat[2]));
            add_value(values, col_sb , face_fac * 1.0 * (theta * aa_s * w_nat[0] + dxinv * theta * cc_s));
            add_value(values, col_sw , face_fac * 1.0 * (theta * aa_s * w_nat[1] - dxinv * theta * cc_s));
            add_value(values, col_sww, face_fac * 1.0 * (theta * aa_s * w_nat[2]));

            // Contribution Delta q
            // face 0
            face_fac = 0.5 * dy * 0.25;
            add_value(values, col_nb  + 1, face_fac * 1.0 * (theta * bb_n * w_nat[0] + dxinv * theta * dd_n));
            add_value(values, col_nw  + 1, face_fac * 1.0 * (theta * bb_n * w_nat[1] - dxinv * theta * dd_n));
            add_value(values, col_nww + 1, face_fac * 1.0 * (theta * bb_n * w_nat[2]));
            add_value(values, col_b   + 1, face_fac * 3.0 * (theta * bb_b * w_nat[0] + dxinv * theta * dd_b));
            add_value(values, col_w   + 1, face_fac * 3.0 * (theta * bb_b * w_nat[1] - dxinv * theta * dd_b));
            ...
            add_value(values, col_w   + 1, face_fac * 3.0 * (theta * bb_b * w_nat[1] - dxinv * theta * dd_b));
            add_value(values, col_ww  + 1, face_fac * 3.0 * (theta * bb_b * w_nat[2]));
            add_value(values, col_sb  + 1, face_fac * 1.0 * (theta * bb_s * w_nat[0] + dxinv * theta * dd_s));
            add_value(values, col_sw  + 1, face_fac * 1.0 * (theta * bb_s * w_nat[1] - dxinv * theta * dd_s));
        ...
        // face 0
        face_fac = 0.5 * dy * 0.25;
        add_value(values, col_nb  + 1, face_fac * 1.0 * con_fac *  dxinv * theta);
        add_value(values, col_nw  + 1, face_fac * 1.0 * con_fac * -dxinv * theta);
        add_value(values, col_nww + 1, face_fac * 1.0 * 0.0);
        add_value(values, col_b   + 1, face_fac * 3.0 * con_fac *  dxinv * theta);
        add_value(values, col_w   + 1, face_fac * 3.0 * con_fac * -dxinv * theta);
        ...
        add_value(values, col_w   + 1, face_fac * 3.0 * con_fac * -dxinv * theta);
        add_value(values, col_ww  + 1, face_fac * 3.0 * 0.0);
        add_value(values, col_sb  + 1, face_fac * 1.0 * con_fac *  dxinv * theta);
        add_value(values, col_sw  + 1, face_fac * 1.0 * con_fac * -dxinv * theta);
    ...
    std::vector<double>& w_nat, std::vector<double>& w_ess)
{
    ...
    else
    {
        ...
        double face_fac = 0.5 * dx * 0.25;
        add_value(values, col_wb , face_fac * 1.0 * w_nat[0] * theta * g * dzetady_e - face_fac * 1.0 * dyinv * theta * g * htheta_e);
        add_value(values, col_wn , face_fac * 1.0 * w_nat[1] * theta * g * dzetady_e + face_fac * 1.0 * dyinv * theta * g * htheta_e);
        add_value(values, col_wnn, face_fac * 1.0 * w_nat[2] * theta * g * dzetady_e);

        add_value(values, col_b , face_fac * 3.0 * w_nat[0] * theta * g * dzetady_b - face_fac * 3.0 * dyinv * theta * g * htheta_b);
        add_value(values, col_n , face_fac * 3.0 * w_nat[1] * theta * g * dzetady_b + face_fac * 3.0 * dyinv * theta * g * htheta_b);
        ...
        add_value(values, col_eb , face_fac * 1.0 * w_nat[0] * theta * g * dzetady_w - face_fac * 1.0 * dyinv * theta * g * htheta_w);
        add_value(values, col_en , face_fac * 1.0 * w_nat[1] * theta * g * dzetady_w + face_fac * 1.0 * dyinv * theta * g * htheta_w);
        add_value(values, col_enn, face_fac * 1.0 * w_nat[2] * theta * g * dzetady_w);
        ...
        if (do_convection)
        {
            ...
            double face_fac = 0.5 * dx * 0.25;
            add_value(values, col_eb , face_fac * 1.0 * (theta * aa_e * w_nat[0] - dyinv * theta * cc_e));
            add_value(values, col_en , face_fac * 1.0 * (theta * aa_e * w_nat[1] + dyinv * theta * cc_e));
            add_value(values, col_enn, face_fac * 1.0 * (theta * aa_e * w_nat[2]));
            add_value(values, col_b  , face_fac * 3.0 * (theta * aa_b * w_nat[0] - dyinv * theta * cc_b));
            add_value(values, col_n  , face_fac * 3.0 * (theta * aa_b * w_nat[1] + dyinv * theta * cc_b));
            ...
            add_value(values, col_nn , face_fac * 3.0 * (theta * aa_b * w_nat[2]));
            add_value(values, col_wb , face_fac * 1.0 * (theta * aa_w * w_nat[0] - dyinv * theta * cc_w));
            add_value(values, col_wn , face_fac * 1.0 * (theta * aa_w * w_nat[1] + dyinv * theta * cc_w));
            ...
            face_fac = 0.5 * dx * 0.25;
            add_value(values, col_eb  + 2, face_fac * 1.0 * (theta * bb_e * w_nat[0] - dyinv * theta * dd_e));
            add_value(values, col_en  + 2, face_fac * 1.0 * (theta * bb_e * w_nat[1] + dyinv * theta * dd_e));
            add_value(values, col_enn + 2, face_fac * 1.0 * (theta * bb_e * w_nat[2]));
            add_value(values, col_b   + 2, face_fac * 3.0 * (theta * bb_b * w_nat[0] - dyinv * theta * dd_b));
            add_value(values, col_n   + 2, face_fac * 3.0 * (theta * bb_b * w_nat[1] + dyinv * theta * dd_b));
            ...
            add_value(values, col_nn  + 2, face_fac * 3.0 * (theta * bb_b * w_nat[2]));
            add_value(values, col_wb  + 2, face_fac * 1.0 * (theta * bb_w * w_nat[0] - dyinv * theta * dd_w));
            add_value(values, col_wn  + 2, face_fac * 1.0 * (theta * bb_w * w_nat[1] + dyinv * theta * dd_w));
        ...
        face_fac = 0.5 * dx * 0.25;
        add_value(values, col_wb  + 2, face_fac * 1.0 * -con_fac * -dyinv * theta);
        add_value(values, col_wn  + 2, face_fac * 1.0 * -con_fac *  dyinv * theta);
        add_value(values, col_wnn + 2, face_fac * 1.0 * 0.0);
        add_value(values, col_b   + 2, face_fac * 3.0 * -con_fac * -dyinv * theta);
        add_value(values, col_n   + 2, face_fac * 3.0 * -con_fac *  dyinv * theta);
        ...
        add_value(values, col_nn  + 2, face_fac * 3.0 * 0.0);
        add_value(values, col_eb  + 2, face_fac * 1.0 * -con_fac *  -dyinv * theta);
        add_value(values, col_en  + 2, face_fac * 1.0 * -con_fac *   dyinv * theta);
    ...
    std::vector<double>& w_nat, std::vector<double>& w_ess)
{
    ...
    else
    {
        ...
        double face_fac = 0.5 * dy * 0.25;
        add_value(values, col_sb , face_fac * 1.0 * w_nat[0] * theta * g * dzetadx_s - face_fac * 1.0 * dxinv * theta * g * htheta_s);
        add_value(values, col_se , face_fac * 1.0 * w_nat[1] * theta * g * dzetadx_s + face_fac * 1.0 * dxinv * theta * g * htheta_s);
        add_value(values, col_see, face_fac * 1.0 * w_nat[2] * theta * g * dzetadx_s);
        add_value(values, col_b  , face_fac * 3.0 * w_nat[0] * theta * g * dzetadx_b - face_fac * 3.0 * dxinv * theta * g * htheta_b);
        add_value(values, col_e  , face_fac * 3.0 * w_nat[1] * theta * g * dzetadx_b + face_fac * 3.0 * dxinv * theta * g * htheta_b);
        ...
        add_value(values, col_ee , face_fac * 3.0 * w_nat[2] * theta * g * dzetadx_b);
        add_value(values, col_nb , face_fac * 1.0 * w_nat[0] * theta * g * dzetadx_n - face_fac * 1.0 * dxinv * theta * g * htheta_n);
        add_value(values, col_ne , face_fac * 1.0 * w_nat[1] * theta * g * dzetadx_n + face_fac * 1.0 * dxinv * theta * g * htheta_n);
        ...
        if (do_convection)
        {
            ...
            double face_fac = 0.5 * dy * 0.25;
            add_value(values, col_sb , face_fac * 1.0 * (theta * aa_s * w_nat[0] + dxinv * theta * cc_s));
            add_value(values, col_se , face_fac * 1.0 * (theta * aa_s * w_nat[1] - dxinv * theta * cc_s));
            add_value(values, col_see, face_fac * 1.0 * (theta * aa_s * w_nat[2]));
            add_value(values, col_b  , face_fac * 3.0 * (theta * aa_b * w_nat[0] + dxinv * theta * cc_b));
            add_value(values, col_ee , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dxinv * theta * cc_b));
            add_value(values, col_ee , face_fac * 3.0 * (theta * aa_b * w_nat[2]));

            //face 1
            add_value(values, col_b  , face_fac * 3.0 * (theta * aa_b * w_nat[0] + dxinv * theta * cc_b));
            add_value(values, col_e  , face_fac * 3.0 * (theta * aa_b * w_nat[1] - dxinv * theta * cc_b));
            add_value(values, col_ee , face_fac * 3.0 * (theta * aa_b * w_nat[2]));
            add_value(values, col_nb , face_fac * 1.0 * (theta * aa_n * w_nat[0] + dxinv * theta * cc_n));
            add_value(values, col_nee, face_fac * 1.0 * (theta * aa_n * w_nat[1] - dxinv * theta * cc_n));
            ...
            face_fac = 0.5 * dy * 0.25;
            add_value(values, col_sb  + 1, face_fac * 1.0 * (theta * bb_s * w_nat[0] - dxinv * theta * dd_s));
            add_value(values, col_se  + 1, face_fac * 1.0 * (theta * bb_s * w_nat[1] + dxinv * theta * dd_s));
            add_value(values, col_see + 1, face_fac * 1.0 * (theta * bb_s * w_nat[2]));
            add_value(values, col_b   + 1, face_fac * 3.0 * (theta * bb_b * w_nat[0] - dxinv * theta * dd_b));
            add_value(values, col_e   + 1, face_fac * 3.0 * (theta * bb_b * w_nat[1] + dxinv * theta * dd_b));
            ...
            add_value(values, col_ee  + 1, face_fac * 3.0 * (theta * bb_b * w_nat[2]));
            add_value(values, col_nb  + 1, face_fac * 1.0 * (theta * bb_n * w_nat[0] - dxinv * theta * dd_n));
            add_value(values, col_ne  + 1, face_fac * 1.0 * (theta * bb_n * w_nat[1] + dxinv * theta * dd_n));
        ...
        face_fac = 0.5 * dy * 0.25;
        add_value(values, col_sb  + 1, face_fac * 1.0 * -con_fac * -dxinv * theta);
        add_value(values, col_se  + 1, face_fac * 1.0 * -con_fac *  dxinv * theta);
        add_value(values, col_see + 1, face_fac * 1.0 * 0.0);
        add_value(values, col_b   + 1, face_fac * 3.0 * -con_fac * -dxinv * theta);
        add_value(values, col_e   + 1, face_fac * 3.0 * -con_fac *  dxinv * theta);
        ...
        add_value(values, col_ee  + 1, face_fac * 3.0 * 0.0);
        add_value(values, col_nb  + 1, face_fac * 1.0 * -con_fac * -dxinv * theta);
        add_value(values, col_ne  + 1, face_fac * 1.0 * -con_fac *  dxinv * theta);
