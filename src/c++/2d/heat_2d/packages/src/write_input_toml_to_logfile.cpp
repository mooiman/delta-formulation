...
int write_used_input(struct _data_input data, std::ofstream & log_file){
    ...
    log_file << data.boundary.bc_type[data.boundary.bc_type.size() - 1] << "\"]  # Type \"neumann\", \"dirichlet\"" << std::endl;
    ...
    log_file << "    " << "ini_vars = [\"";
    for (int i = 0; i < data.initial.ini_vars.size(); ++i)
    {
        log_file << data.initial.ini_vars[i];
        if (i < data.initial.ini_vars.size() - 1) { log_file << "\", \""; }
    }
    log_file << "\"]" << std::endl;
