...
_data_input read_toml_file(std::filesystem::path & input_dir, std::filesystem::path & toml_file_name){
    ...
    toml::table tbl = toml::parse_file(toml_file_name.c_str());

    data.log.logging = tbl["Logging"].value_or("None");

    // Boundary
    if (auto* tbl_chp = tbl["Boundary"].as_table())
    {
        ...
        status = get_toml_array((*tbl_chp), "bc_signals", data.boundary.bc_signals);
    ...
    // Initial
    if (auto* tbl_chp = tbl["Initial"].as_table())
    {
    ...
    // Numerics
    if (auto* tbl_chp = tbl["Numerics"].as_table())
    {
    ...
    // Output
    if (auto* tbl_chp = tbl["Output"].as_table())
    {
    ...
    //Physics
    if (auto* tbl_chp = tbl["Physics"].as_table())
    {
    ...
    // Time
    if (auto* tbl_chp = tbl["Time"].as_table())
    {
